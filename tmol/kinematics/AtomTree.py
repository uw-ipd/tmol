...
class HomogeneousTransform:
    ...
    def inverse(self):
        ...
        ht_res.frame[0:3, 3] = -1 * numpy.dot(
            self.frame[0:3, 3], self.frame[0:3, 0:3]
        )
        return ht_res
...
class BondedAtom(TreeAtom):
    ...
    def update_internal_coords(self, parent_ht=None):
        ...
        new_ht = parent_ht * HomogeneousTransform.zrot(
            self.theta
...
class JumpAtom(TreeAtom):
    ...
    def get_ht_from_childrens_coords(self):
        if self.stub_defined():
            ...
            p3 = nonjump_children[1].xyz if len(nonjump_children) > 1 else [
                child.xyz
                for child in nonjump_children[0].children
...
def chi_interruption(residue_type, done, query_at1, query_at2):
    ...
    for at1, at2, at3, at4 in residue_type.chi_inds:
        if query_at2 == at2 and query_at1 != at1 and qeury_at1 != at3:
            return True
        if query_at2 == at3 and query_at1 != at2 and qeury_at1 != at4:
            return True
...
def setup_atom_links(
        root_atom_index, full_links, done, chem_db, residue_type, new_links
):
    ...
    if n_neighbs == 1:
        if not done[neighbs[0]]:
            ...
            setup_atom_links(
                neighbs[0], full_links, done, chem_db, residue_type, new_links
            )
        return
    ...
    for neighb in neighbs:
        ...
        if neighb in residue_type.mainchain_inds:
            ...
            setup_atom_links(
                neighb, full_links, done, chem_db, residue_type, new_links
            )
    # next priority: within a chi angle and heavy
    root_is_chi = is_chi(residue_type, root_atom_index)
    for neighb in neighbs:
        if done[neighb]: continue
        if root_is_chi and is_chi(residue_type, neighb) and chi_continuation(
                residue_type, root_atom_index, neighb
        ) and not is_hydrogen(chem_db, residue_type, neighb):
            root_links.append(neighb)
            setup_atom_links(
                neighb, full_links, done, chem_db, residue_type, new_links
            )
    ...
    for neighb in neighbs:
        if done[neighb]: continue
        if is_chi(residue_type, neighb) and chi_interruption(
                residue_type, done, root_atom_index, neighb):
            continue
...
def add_atom(root_atom_index, ordered_links, atom_pointers):
    ...
    for child in ordered_links[root_atom_index]:
        ...
        tree_node.children.append(
            add_atom(child, ordered_links, atom_pointers)
        )
...
def create_residue_tree(chem_db, residue_type, res_index, root_atom):
    ...
    setup_atom_links(
        root_atom_index, full_links, done, chem_db, residue_type, ordered_links
    )
...
def tree_from_residues(chem_db, residues):
    ...
    for ind, residue in enumerate(residues):
        ...
        if last_residue:
            upper_connect_last = atom_pointers_list[-1][
                last_residue.residue_type.atom_to_idx[
                    last_residue.residue_type.upper_connect
                ]
            ]
