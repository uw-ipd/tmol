...
def bfs_proper_forest(
    ...
    n_res: NDArray[numpy.int64][:],
    connections: NDArray[numpy.int64][:, :, :],
):
    n_poses = connections.shape[0]
    max_n_res = connections.shape[1]
    assert n_res.shape[0] == n_poses
    assert roots.shape[0] == n_poses

    cycles_detected = numpy.zeros((n_poses, 2), dtype=numpy.int64)
    missing = numpy.zeros((n_poses, max_n_res), dtype=numpy.int64)

    for i in range(n_poses):
        found_cycle = False
        node_colors = numpy.zeros((max_n_res,), dtype=numpy.int64)
        visit_queue = numpy.full((max_n_res,), -1, dtype=numpy.int64)
        visit_front = 0
        visit_back = 0
...
def validate_fold_forest(
    n_blocks: NDArray[numpy.int64][:], edges: NDArray[numpy.int64][:, :, 4]
):
