...
class view(object):
    ...
    def __init__(
        ...
        if viewergrid:  #create split view
            if len(viewergrid) != 2:
                raise ValueError(
                    "Incorrectly formated viewergrid arguments.  Must specify rows x columns",
                    viewergrid
                )
            ...
            self.startjs += 'viewergrid_UNIQUEID = $3Dmol.createViewerGrid($("#%s"),{rows: %d, cols: %d, control_all: %s},{backgroundColor:"white"});\n' % (
                divid, viewergrid[0], viewergrid[1], 'true'
    ...
    def insert(self, containerid):
        ...
        html += '''<script>$("#%s").append($("#3dmolviewer_%s")); </script>''' % (
            containerid, self.uniqueid
        )
    ...
    def update(self):
        ...
        script += '''<script>
            ...
            self.updatejs.replace('UNIQUEID', self.uniqueid), self.uniqueid
        )
        self.updatejs = ''
    ...
    def png(self):
        ...
        if not self.uniqueid:
            raise AssertionError(
                'Must instantiate viewer before generating image.'
            )
        ...
        def makejs(*args, **kwargs):
            if self.viewergrid:
                if 'viewer' in kwargs:
                    ...
                    if len(coords) != 2 or coords[0] >= self.viewergrid[
                            0
                    ] or coords[1] >= self.viewergrid[1] or coords[
                            0
                    ] < 0 or coords[1] < 0:
                        raise ValueError(
                            "Incorrectly formated viewer argument.  Must specify row and column",
                            self.viewergrid
                        )
                    cmd = '\tviewergrid_UNIQUEID[%d][%d].%s(' % (
