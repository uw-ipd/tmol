...
@validate_args
def sp2chi_energy(d: float, m: float, l: float, BAH: Params, chi: Params):
    ...
    F[selector_upper] = d / 2 * torch.cos(3 * (pi - BAH[selector_upper])) + d / 2 - 0.5
...
def hbond_donor_sp2_score(
    ...
    energy[med_energy_selector] = (
        -0.025
        + 0.5 * energy[med_energy_selector]
