...
template <template <tmol::Device> class DispatchMethod>
class HBondRotamerScoresOp
    : public torch::autograd::Function<HBondRotamerScoresOp<DispatchMethod>> {
 public:
  static std::vector<Tensor> forward(
      AutogradContext* ctx,
      // common params
    ...
    TMOL_DISPATCH_FLOATING_DEVICE(
        rot_coords.options(), "hbond_rotamer_score_op", ([&] {
          using Real = scalar_t;
          constexpr tmol::Device Dev = device_t;
    ...
    if (saved.size() == 2) {
      ...
      TMOL_DISPATCH_FLOATING_DEVICE(
          rot_coords.options(), "hbond_pose_score_backward", ([&] {
            ...
            auto result = HBondRotamerScoreDispatch<
                common::DeviceOperations,
                Dev,
                Real,
                Int>::
...
template <template <tmol::Device> class DispatchMethod>
std::vector<Tensor> hbond_rotamer_scores_op(
    // common params
    Tensor rot_coords,
    Tensor rot_coord_offset,
    Tensor pose_ind_for_atom,
