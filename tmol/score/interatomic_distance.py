...
class BlockedDistanceAnalysis(properties.HasProperties):
    ...
    coord_mask = VariableT("mask of coordinates to consider in interaction calculation")

    block_centers = VariableT("block mean coordinate")
    block_radii = VariableT("block mean coordinate interaction distances")

    block_triu_inds = VariableT("triu indicies of the inter-block interaction matrix")
    block_triu_dist = VariableT("inter-block mean coordinate interaction distances")
    block_triu_min_dist = VariableT("inter-block minimum member coordinate interaction distance")
    ...
    def setup(cls, block_size, coords):
        ...
        block_triu_inds = torch.stack(tuple(map(torch.LongTensor,
        ...
        block_triu_dist = (block_centers[block_triu_ind[0]] - block_centers[block_triu_ind[1]]).norm(dim=-1)
...
class BlockedInteratomicDistanceGraph(InteratomicDistanceGraphBase):
    ...
    @derived_from(
        ("coords", "atom_pair_block_size"),
        properties.Instance(
            "current inter-block distance analysis",
            instance_class=BlockedDistanceAnalysis))
    def interblock_analysis(self):
        return BlockedDistanceAnalysis.setup(self.atom_pair_block_size, self.coords.detach())

    @derived_from("interblock_analysis",
    ...
    def atom_pair_inds(self):
        ...
        interblock_triu_matches = ba.block_triu_inds[:,
              (ba.block_triu_min_dist < self.interatomic_threshold_distance).nonzero().squeeze(dim=-1)]
