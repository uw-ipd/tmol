...
class AtomTypeDependentTerm(EnergyTerm):
    ...
    def __init__(self, param_db: ParameterDatabase, device: torch.device):
        atom_type_resolver = AtomTypeParamResolver.from_database(param_db.chemical, device=device)
    ...
    def _create_uniq_and_wildcard_names_for_bt(self, block_type):
        ...
        unique_ids = [self.get_atom_unique_id_name(block_type.base_name, atom_name) for atom_name in atom_names]
        wildcard_ids = [self.get_atom_wildcard_id_name(atom_name) for atom_name in atom_names]
    ...
    def setup_block_type(self, block_type: RefinedResidueType):
        ...
        unique_ids, wildcard_ids = self._create_uniq_and_wildcard_names_for_bt(block_type)

        atom_types = self.atom_type_index.get_indexer([x.atom_type for x in block_type.atoms])
    ...
    def setup_packed_block_types(self, packed_block_types: PackedBlockTypes):
        ...
        for i, restype in enumerate(packed_block_types.active_block_types):
            atom_types[i, : packed_block_types.n_atoms[i]] = self.atom_type_index.get_indexer(
        ...
        n_heavy_atoms = numpy.array([len(heavy_inds) for heavy_inds in heavy_atom_inds], dtype=numpy.int32)
        max_n_heavy = numpy.max(n_heavy_atoms)

        heavy_atom_inds_t = torch.full((packed_block_types.n_types, max_n_heavy), -1, dtype=torch.int32)
        ...
        n_heavy_atoms = torch.tensor(n_heavy_atoms, dtype=torch.int32, device=self.device)
