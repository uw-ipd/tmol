...
template <template <tmol::Device> class DispatchMethod>
class DisulfideRotamerScoreOp : public torch::autograd::Function<
                                    DisulfideRotamerScoreOp<DispatchMethod>> {
 public:
  static std::vector<Tensor> forward(
      AutogradContext* ctx,
    ...
    TMOL_DISPATCH_FLOATING_DEVICE(
        rot_coords.options(), "disulfide_rotamer_score_op", ([&] {
          using Real = scalar_t;
          constexpr tmol::Device Dev = device_t;
    ...
    conns_for_dispatch_indices = mps_to_dev(conns_for_dispatch_indices, orig_device);
  ...
  static tensor_list backward(AutogradContext* ctx, tensor_list grad_outputs) {
    ...
    if (saved.size() == 2) {
      ...
      TMOL_DISPATCH_FLOATING_DEVICE(
          rot_coords.options(), "disulfide_rotamer_score_backward", ([&] {
            using Real = scalar_t;
            constexpr tmol::Device Dev = device_t;

            auto result = DisulfideRotamerScoreDispatch<
                common::DeviceOperations,
                Dev,
                Real,
                Int>::
...
template <template <tmol::Device> class DispatchMethod>
std::vector<Tensor> disulfide_pose_scores_op(
    // common params
    Tensor rot_coords,
    Tensor rot_coord_offset,
    Tensor pose_ind_for_atom,
...
template <template <tmol::Device> class DispatchMethod>
std::vector<Tensor> disulfide_rotamer_scores_op(
    // common params
    Tensor rot_coords,
    Tensor rot_coord_offset,
    Tensor pose_ind_for_atom,
    ...
    bool output_block_pair_energies) {
  return DisulfideRotamerScoreOp<DispatchMethod>::apply(
      // common params
      rot_coords,
      rot_coord_offset,
      pose_ind_for_atom,
...
TORCH_LIBRARY(tmol_disulfide, m) {
  ...
  m.def(
      ...
      &disulfide_rotamer_scores_op<DeviceOperations>);
}
