...
class GenBondedEnergyTerm(AtomTypeDependentTerm):
    ...
    def __init__(self, param_db: ParameterDatabase, device: torch.device):
        ...
        self._block_annotation_key = AnnotationKey.from_sources(
            param_db.chemical, self.gen_database
        )
        self._packed_annotation_key = AnnotationKey.from_sources(
            param_db.chemical, self.gen_database, settings=(self.device,)
        )
    ...
    def atom_hierarchy_indices(self, atom_type: str) -> List[int]:
        ...
        if len(hierarchy) > MAX_HIER_DEPTH:
            # Silently dropping the most generic levels makes a torsion that
            # only matches there score zero between blocks while scoring
            # normally within one. The kernels carry GB_MAX_HIER_DEPTH entries;
            # say so rather than truncate.
            raise ValueError(
                f"{atom_type}: generic-bonded hierarchy is {len(hierarchy)} deep, "
                f"but the scoring kernels carry {MAX_HIER_DEPTH}. Raise "
                "MAX_HIER_DEPTH and GB_MAX_HIER_DEPTH together, or shorten the "
                "chain in scoring/genbonded.yaml"
            )
        result = []
        for ht in hierarchy:
    ...
    def setup_block_type(self, block_type: RefinedResidueType):
        ...
        previous = cached_annotation(
            block_type, "_genbonded_annotation", self._block_annotation_key
        )
        ...
        annotation = (intra_subgraphs, intra_params, hier_arr)
    ...
    def setup_packed_block_types(  # noqa: C901
        ...
        block_values = [self.setup_block_type(bt) for bt in block_types]
        ...
        for bt_idx, bt in enumerate(block_types):
            ...
            for conn, paths in enumerate(bt.atom_paths_from_conn):
                conn_bond_bins[bt_idx, conn, 0] = _bond_btidx(
                    int(bt.connection_bond_types[conn]),
                    bool(bt.connection_bond_in_ring[conn]),
                )
