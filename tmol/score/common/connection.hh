...
// Given an integer, returns a tuple of three integers, a type and two indices
// into the atom_paths_from_conn tensor. Using these indices on two connected
...
TMOL_DEVICE_FUNC tuple<Int, Int, Int> get_connection_spanning_subgraph_indices(
  ...
  return {
      ...
      CON_PATH_INDICES[index][2]};
}

inline TMOL_DEVICE_FUNC int get_n_connection_spanning_subgraphs(
    subgraph_type st) {
  ...
  switch (st) {
    case subgraph_length: {
      return 1;
    }
    case subgraph_angle: {
      return 6;
    }
    case subgraph_torsion: {
      return 27;
    }
  };
}

inline TMOL_DEVICE_FUNC int get_connection_spanning_subgraphs_offset(
    subgraph_type st) {
  switch (st) {
    case subgraph_length: {
      return 0;
