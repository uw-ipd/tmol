...
struct compute_block_spheres {
  ...
  static void f(
    ...
    DeviceDispatch<D>::template foreach_workgroup<launch_t>(
        context, block_ind_for_rot.size(0), compute_spheres);
  }
};
...
struct detect_block_neighbors {
  static void f(
    ...
    auto detect_neighbors = ([=] TMOL_DEVICE_FUNC(int ind) {
      ...
      // block_neighbors[pose_ind][block_ind1][block_ind2]);
    });
  ...
  static void f(
    ...
    DeviceDispatch<D>::template forall<launch_t>(
        context, n_block_pairs, detect_neighbors);
  }
};
...
struct asynch_block_neighbor_indices {
  ...
  static void f(
    ...
    auto fill_indices = ([=] TMOL_DEVICE_FUNC(int ind) {
      int const pose = ind / (n_res * n_res);
      int const ind_r1r2 = ind % (n_res * n_res);
      int const res1 = ind_r1r2 / n_res;
      int const res2 = ind_r1r2 % n_res;

      if (block_neighbors[pose][res1][res2]) {
        int const offset = block_neighbor_offsets[ind];
