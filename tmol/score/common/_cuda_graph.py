    ...
    def __init__(self, module: torch.nn.Module, example_coords: torch.Tensor):
        ...
        hooked = next(
            (
                ...
                if child._backward_hooks or child._forward_hooks or child._forward_pre_hooks
            ),
            None,
        )
        if hooked is not None:
            # A hook on any submodule is as invisible to the replay as one on the root.
            raise AssertionError(f"Scoring modules must not have hooks before capture: {hooked}")
