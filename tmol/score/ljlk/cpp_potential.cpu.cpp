...
at::Tensor block_interaction_lists(
  ...
  TView<Box, 2> boxes = tmol::view_tensor<Box, 2>(boxes_t);

  at::Tensor block_lists_t = at::empty(
      {num_blocks, num_blocks},
      at::TensorOptions(coords_t).dtype(at::CTypeToScalarType<Int>::to()));
  tmol::TView<Int, 2> block_lists = tmol::view_tensor<Int, 2>(block_lists_t);
  ...
  for (int bi = 0; bi < num_blocks; ++bi) {
    ...
    for (int i = bsi + 1; i < bsi + block_size; ++i) {
  ...
  at::Tensor result_t = at::empty(
      {block_spans_t[num_blocks].toCLong(), 2},
      at::TensorOptions(block_spans_t));

  TView<Int, 1> block_spans = tmol::view_tensor<Int, 1>(block_spans_t);
...
at::Tensor lj_intra_block(
  ...
  TView<Vector, 2> coords = tmol::view_tensor<Vector, 2>(coords_t);
  TView<Int, 2> block_interactions =
      tmol::view_tensor<Int, 2>(block_interactions_t);
  TView<Int, 1> types = tmol::view_tensor<Int, 1>(types_t);
