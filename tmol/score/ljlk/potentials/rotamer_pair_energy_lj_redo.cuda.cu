...
auto LJRPEDispatch<DeviceDispatch, D, Real, Int>::f(
  ...
  auto score_inter_pairs = ([=] MGPU_DEVICE(
                                ...
                                ubyte const *alt_path_dist,     // shared
                                ubyte const *neighb_path_dist,  // shared
                                ubyte const *conn_seps) {       // shared
    Real score_total = 0;
    Real coord1[3];
    Real coord2[3];
  ...
  auto eval_energies = ([=] MGPU_DEVICE(int tid, int cta) {
    ...
    __shared__ struct shared_mem_struct {
      ...
      ubyte conn_ats_alt1[MAX_N_CONN];  // 8 bytes for conn ats
      ubyte conn_ats_alt2[MAX_N_CONN];
      ubyte path_dist_alt1[MAX_N_CONN * TILE_SIZE];  // 256 bytes for path dists
      ubyte path_dist_alt2[MAX_N_CONN * TILE_SIZE];

      union {
        struct {
          Real coords_other[TILE_SIZE * 3];            // 384 bytes for coords
          LJTypeParams<Real> params_other[TILE_SIZE];  // 768 bytes for params
          ubyte conn_ats_other[MAX_N_CONN];            // 4 bytes
          ubyte path_dist_other[MAX_N_CONN * TILE_SIZE];  // 128 bypes
          ubyte conn_seps[MAX_N_CONN * MAX_N_CONN];  // 64 bytes for conn/conn
        } vals;
    ...
    for (int ivt = 0; ivt < vt; ++ivt) {
      ...
      int ivt_context_ind = (vt * cta + ivt) / max_n_neighbors;
      ...
      if (alt_block_ind != neighb_block_ind) {
        ...
        for (int i = 0; i < alt_n_iterations; ++i) {
          ...
          if (new_alt || alt_n_atoms1 > TILE_SIZE) {
            mgpu::mem_to_shared<TILE_SIZE, 3>(
                reinterpret_cast<Real *>(
                    &alternate_coords[rot_ind1][4 + i * TILE_SIZE]),
                tid,
          ...
          if (new_alt || alt_n_atoms2 > TILE_SIZE) {
            mgpu::mem_to_shared<TILE_SIZE, 3>(
                reinterpret_cast<Real *>(
                    &alternate_coords[rot_ind2][4 + i * TILE_SIZE]),
                tid,
