...
auto LJLKPoseScoreDispatch<DeviceDispatch, D, Real, Int>::forward(
  ...
  //   STORE_CALCULATED_ENERGIES_BLOCKPAIR);
  //
  ...
  auto eval_energies_by_block = ([=] TMOL_DEVICE_FUNC(int cta) {
    ...
    if (block_ind1 != block_ind2) {
      ...
      int const n_atoms1 = block_type_n_atoms[block_type1];
      int const n_atoms2 = block_type_n_atoms[block_type2];
      int const n_atom_pairs = n_atoms1 * n_atoms2;
      int const n_hvy_atoms1 = block_type_n_heavy_atoms[block_type1];
      int const n_hvy_atoms2 = block_type_n_heavy_atoms[block_type2];
      int const n_heavy_atom_pairs = n_hvy_atoms1 * n_hvy_atoms2;
      int const coord_offset1 =
          pose_stack_block_coord_offset[pose_ind][block_ind1];
      int const coord_offset2 =
      ...
      LJGlobalParams<Real> my_global_params = global_params[0];

      auto eval_lj_interres_atom_pairs = ([=] TMOL_DEVICE_FUNC(int tid) {
        Real total_ljatr = 0;
        Real total_ljrep = 0;
        for (int i = tid; i < n_atom_pairs; i += nt) {
          int const atom1 = i / n_atoms2;
          int const atom2 = i % n_atoms2;

          int const atom_type1 = block_type_atom_types[block_type1][atom1];
          int const atom_type2 = block_type_atom_types[block_type2][atom2];

          LJLKTypeParams<Real> atom_params1 = type_params[atom_type1];
          LJLKTypeParams<Real> atom_params2 = type_params[atom_type2];

          int bond_separation = max_important_bond_separation + 1;
          if (min_separation <= max_important_bond_separation) {
            for (int conn1 = 0; conn1 < n_conn1; ++conn1) {
              int const conn1_atom =
                  block_type_atoms_forming_chemical_bonds[block_type1][conn1];
              int const dist_to_conn1 =
                  block_type_path_distance[block_type1][atom1][conn1_atom];

              for (int conn2 = 0; conn2 < n_conn2; ++conn2) {
                int const conn2_atom =
                    block_type_atoms_forming_chemical_bonds[block_type2][conn2];
                int const dist_to_conn2 =
                    block_type_path_distance[block_type2][atom2][conn2_atom];
                int const conn1_conn2_sep =
                    (dist_to_conn1 + dist_to_conn2
                     + pose_stack_inter_block_bondsep[pose_ind][block_ind1]
                                                     [block_ind2][conn1]
                                                     [conn2]);
                if (conn1_conn2_sep < bond_separation) {
                  bond_separation = conn1_conn2_sep;
                }
              }
            }
          }

          Real3 coord1 = coords[pose_ind][coord_offset1 + atom1];
          Real3 coord2 = coords[pose_ind][coord_offset2 + atom2];

          Real const dist = distance<Real>::V(coord1, coord2);
          std::array<Real, 2> energy = lj_score<Real>::V(
              dist,
              bond_separation,
              atom_params1.lj_params(),
              atom_params2.lj_params(),
              my_global_params);

          total_ljatr += energy[0];
          total_ljrep += energy[1];
        }
        DeviceDispatch<D>::synchronize_workgroup();
        Real cta_total_ljatr =
            DeviceDispatch<D>::template shuffle_reduce_in_workgroup<nt>(
                total_ljatr, mgpu::plus_t<Real>());
        Real cta_total_ljrep =
            DeviceDispatch<D>::template shuffle_reduce_in_workgroup<nt>(
                total_ljrep, mgpu::plus_t<Real>());
        if (tid == 0) {
          if (output_block_pair_energies) {
            output[0][pose_ind][block_ind1][block_ind2] = cta_total_ljatr;
            output[1][pose_ind][block_ind1][block_ind2] = cta_total_ljrep;
          } else {
            accumulate<D, Real>::add(
                output[0][pose_ind][0][0], cta_total_ljatr);
            accumulate<D, Real>::add(
                output[1][pose_ind][0][0], cta_total_ljrep);
          }
        }
      });
      DeviceDispatch<D>::template for_each_in_workgroup<nt>(
          eval_lj_interres_atom_pairs);

      auto eval_lk_interres_atom_pairs = ([=] TMOL_DEVICE_FUNC(int tid) {
        Real total_lk = 0;
        for (int i = tid; i < n_heavy_atom_pairs; i += nt) {
          int const hvy_atom1 = i / n_hvy_atoms2;
          int const hvy_atom2 = i % n_hvy_atoms2;

          int const atom1 = block_type_heavy_atom_inds[block_type1][hvy_atom1];
          int const atom2 = block_type_heavy_atom_inds[block_type2][hvy_atom2];
          ...
          Real const dist = distance<Real>::V(coord1, coord2);
          Real lk_energy = lk_isotropic_score<Real>::V(
              dist,
              bond_separation,
              atom_params1.lk_params(),
              atom_params2.lk_params(),
              my_global_params);
        ...
        Real cta_total_lk =
            DeviceDispatch<D>::template shuffle_reduce_in_workgroup<nt>(
                total_lk, mgpu::plus_t<Real>());

        if (tid == 0) {
          if (output_block_pair_energies) {
            output[2][pose_ind][block_ind1][block_ind2] = cta_total_lk;
          } else {
            accumulate<D, Real>::add(output[2][pose_ind][0][0], cta_total_lk);
          }
        }
      });
      DeviceDispatch<D>::template for_each_in_workgroup<nt>(
          eval_lk_interres_atom_pairs);

    } else {
      int const n_atoms1 = block_type_n_atoms[block_type1];
      int const n_atom_pairs = n_atoms1 * n_atoms1;
      int const n_hvy_atoms1 = block_type_n_heavy_atoms[block_type1];
      int const n_heavy_atom_pairs = n_hvy_atoms1 * n_hvy_atoms1;
      int const coord_offset1 =
          pose_stack_block_coord_offset[pose_ind][block_ind1];

      LJGlobalParams<Real> my_global_params = global_params[0];

      auto eval_lj_intrares_atom_pairs = ([=] TMOL_DEVICE_FUNC(int tid) {
        Real total_ljatr = 0;
        Real total_ljrep = 0;
        for (int i = tid; i < n_atom_pairs; i += nt) {
          int const atom1 = i / n_atoms1;
          int const atom2 = i % n_atoms1;
          if (atom1 >= atom2) {
            continue;
          }
          int const atom_type1 = block_type_atom_types[block_type1][atom1];
          int const atom_type2 = block_type_atom_types[block_type1][atom2];
          LJLKTypeParams<Real> atom_params1 = type_params[atom_type1];
          LJLKTypeParams<Real> atom_params2 = type_params[atom_type2];
          int const bond_separation =
              block_type_path_distance[block_type1][atom1][atom2];
          Real3 coord1 = coords[pose_ind][coord_offset1 + atom1];
          Real3 coord2 = coords[pose_ind][coord_offset1 + atom2];
        ...
        if (tid == 0) {
          if (output_block_pair_energies) {
            output[0][pose_ind][block_ind1][block_ind1] = cta_total_ljatr;
            output[1][pose_ind][block_ind1][block_ind1] = cta_total_ljrep;
          } else {
            accumulate<D, Real>::add(
                output[0][pose_ind][0][0], cta_total_ljatr);
            accumulate<D, Real>::add(
      ...
      DeviceDispatch<D>::template for_each_in_workgroup<nt>(
          eval_lj_intrares_atom_pairs);

      auto eval_lk_intrares_atom_pairs = ([=] TMOL_DEVICE_FUNC(int tid) {
        Real total_lk = 0;
        for (int i = tid; i < n_heavy_atom_pairs; i += nt) {
          int const hvy_atom1 = i / n_hvy_atoms1;
          int const hvy_atom2 = i % n_hvy_atoms1;

          if (hvy_atom1 >= hvy_atom2) {
            continue;
          }
          int const atom1 = block_type_heavy_atom_inds[block_type1][hvy_atom1];
          int const atom2 = block_type_heavy_atom_inds[block_type1][hvy_atom2];
