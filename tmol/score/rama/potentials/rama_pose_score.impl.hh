...
class RamaPoseScoreDispatch {
 public:
  static auto forward(
      ...
      //
      TView<Int, 2, Dev> block_type_atom_downstream_of_conn,

      // The 8 atoms that define the two torsions
      TView<UnresolvedAtomID<Int>, 2, Dev> block_type_rama_torsion_atoms,
      //////////////////////

      // Rama potential parameters
      TView<RamaGlobalParams<Real>, 1, Dev> global_params)
    ...
    auto func = ([=] EIGEN_DEVICE_FUNC(int ind) {
      ...
      int const rama_table_ind = bt_rama_table[block_type];
      if (rama_table_ind < 0) {
        return;
      }
      ...
      Vec<Real3, 4> phi_coords;
      for (int i = 0; i < 4; ++i) {
        phi_coords[i] = coords[pose_ind][phi_ats[i]];
      }
      Vec<Int, 4> psi_ats;
      for (int i = 0; i < 4; ++i) {
        UnresolvedAtomID<Int> i_at =
      ...
      Vec<Real3, 4> p2i_coords;
      for (int i = 0; i < 4; ++i) {
        psi_coords[i] = coords[pose_ind][psi_ats[i]];
      }
  ...
  static auto backward(
      ...
      auto eval_derivs = ([=] TMOL_DEVICE_FUNC(int cta) {
        ...
        auto dscore_intra_lk_ball_atom_pair =
            ([=] TMOL_DEVICE_FUNC(
              ...
              int const separation =
                  block_type_path_distance[intra_dat.r1.block_type]
                                          [pol_atom_ind][occ_atom_ind];
        ...
        auto load_tile_invariant_interres_data = ([=](int pose_ind,
                                                      int block_ind1,
                                                      int block_ind2,
                                                      int block_type1,
                                                      int block_type2,
        ...
        auto load_tile_invariant_intrares_data = ([=](int pose_ind,
                                                      int block_ind1,
                                                      int block_type1,
                                                      int n_atoms1,
                                                      LKBallScoringData<Real>
