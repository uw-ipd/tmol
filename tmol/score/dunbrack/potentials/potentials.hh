...
def measure_dihedral_V_dV(
    ...
    Real dih_default,
    Real& dihedral,
    Eigen::Matrix<Real, 4, 3>& ddihe_dxyz) -> void {
  Int at1 = dihedral_atom_inds[0];
  Int at2 = dihedral_atom_inds[1];
  Int at3 = dihedral_atom_inds[2];
  Int at4 = dihedral_atom_inds[3];
  if (at1 >= 0) {
    auto dihe = dihedral_angle<Real>::V_dV(
        coordinates[at1], coordinates[at2], coordinates[at3], coordinates[at4]);
    dihedral = dihe.V;

    ddihe_dxyz.row(0) = dihe.dV_dI;
    ddihe_dxyz.row(1) = dihe.dV_dJ;
...
def measure_dihedrals_V_dV(
    ...
    TensorAccessor<Eigen::Matrix<Real, 4, 3>, 1, D> ddihe_dxyz) -> void {
...
def classify_rotamer(TensorAccessor<Real, 1, D> dihedrals, int n_rotameric_chi)
    -> int {
...
def classify_rotamer_for_block(
    TensorAccessor<Real, 1, D> dihedrals,
    const Int& nrotameric_chi_for_res,
    const Int& rotind2tableind_offset_for_res,
    TView<Int, 1, D> rotameric_rotind2tableind,
    TView<Int, 1, D> semirotameric_rotind2tableind,
    Int& rotameric_rottable_assignment,
    Int& semirotameric_rottable_assignment) -> void {
  ...
  rotameric_rottable_assignment = rotameric_table_ind;
  semirotameric_rottable_assignment = semirotameric_table_ind;
}
...
def block_interpolate_rotameric_tables(
    ...
    Int table_set,
    Int block_rotmean_table_offset,
    Int block_nchi,

    TensorAccessor<Real, 1, D> dihedrals,
    int chi_ind,
    Int rottable_assignment)
    -> tuple<
        Real,
        ...
        Eigen::Matrix<Real, NbbP1 - 1, 1> > {
  Int tableind =
      block_rotmean_table_offset + block_nchi * rottable_assignment + chi_ind;
  ...
  for (int ii = 0; ii < NbbP1 - 1; ++ii) {
    Real wrap_iidihe = dihedrals[ii] - rotameric_bb_start[table_set][ii];
    while (wrap_iidihe < 0) {
      wrap_iidihe += 2 * M_PI;
    }
...
template <size_t NbbP1, typename Real, typename Int, tmol::Device D>
def block_chi_deviation_penalty(
    TView<Real, NbbP1, D> rotameric_mean_tables,
    TView<Real, NbbP1, D> rotameric_sdev_tables,
    TView<Vec<int64_t, (int)NbbP1 - 1>, 1, D> rotmean_table_sizes,
    TView<Vec<int64_t, (int)NbbP1 - 1>, 1, D> rotmean_table_strides,
  ...
  Real mean, sdev;
  Eigen::Matrix<Real, NbbP1 - 1, 1> dmean_dbb, dsdev_dbb;
  tie(mean, dmean_dbb, sdev, dsdev_dbb) = block_interpolate_rotameric_tables(
...
template <size_t NbbP1, typename Real, typename Int, tmol::Device D>
def block_deviation_penalty_for_chi(
    // From DB
    TView<Real, NbbP1, D> rotameric_mean_tables,
    TView<Real, NbbP1, D> rotameric_sdev_tables,
    TView<Vec<int64_t, (int)NbbP1 - 1>, 1, D> rotmean_table_sizes,
  ...
  tie(devpen, dpen_dchi, dpen_dbb) = block_chi_deviation_penalty(
      rotameric_mean_tables,
      rotameric_sdev_tables,
      rotmean_table_sizes,
      rotmean_table_strides,
...
template <size_t NbbP1, typename Real, typename Int, tmol::Device D>
def block_rotameric_chi_probability(
  ...
  Int const res_rottable = prob_table_offset + rotameric_rottable_assignment;
...
template <size_t NbbP1, typename Real, typename Int, tmol::Device D>
def rotameric_chi_probability_for_block(
    TView<Real, NbbP1, D> rotameric_neglnprob_tables,
    TView<Vec<int64_t, (int)NbbP1 - 1>, 1, D> rotprob_table_sizes,
    TView<Vec<int64_t, (int)NbbP1 - 1>, 1, D> rotprob_table_strides,
    TView<Vec<Real, (int)NbbP1 - 1>, 1, D> rotameric_bb_start,
    TView<Vec<Real, (int)NbbP1 - 1>, 1, D> rotameric_bb_step,
    TView<Vec<Real, (int)NbbP1 - 1>, 1, D> rotameric_bb_periodicity,
    Int block_probability_table_offset,
    Int block_rotamer_table_set,
    TensorAccessor<Real, 1, D> dihedrals,
    Int rotameric_rottable_assignment,
  ...
  tie(neglnprobE, dneglnprob_ddihe) = block_rotameric_chi_probability(
      ...
      rotameric_bb_step,
      rotameric_bb_periodicity,
      block_probability_table_offset,
      dihedrals,
...
def block_semirotameric_energy(
    ...
    Int block_table_set,
    Int block_table_offset,
    Int block_semirot_dihedral_index,  // the last dihedral in a semirot residue
  ...
  Int const semirot_table_offset =
      block_table_offset;  // semirotameric_chi_desc[i][2];
  Int const semirot_table_set =
      block_table_set;  // semirotameric_chi_desc[i][3];

  Int table_ind = semirotameric_rottable_assignment + semirot_table_offset;

  for (int ii = 0; ii < NbbP2 - 1; ++ii) {
    int ii_dihe_ind = ii == NbbP2 - 2 ? block_semirot_dihedral_index : ii;
    Real wrap_iidihe =
