...
struct CartBondedLengthDispatch {
  static auto f(TView<Vec<Int, 2>, 1, D> atompair_indices,
                TView<Int, 1, D> parameter_indices,
                TView<Vec<Real, 3>, 1, D> coords, TView<Real, 1, D> K,
                TView<Real, 1, D> x0)
      ->std::tuple<TPack<Real, 1, D>, TPack<Vec<Real, 3>, 1, D>,
                   TPack<Vec<Real, 3>, 1, D> > {
    auto num_Vs = parameter_indices.size(0);

    auto Vs_t = TPack<Real, 1, D>::empty({
      num_Vs
    });
...
struct CartBondedAngleDispatch {
  static auto f(TView<Vec<Int, 3>, 1, D> atomtriple_indices,
                TView<Int, 1, D> parameter_indices,
                TView<Vec<Real, 3>, 1, D> coords, TView<Real, 1, D> K,
                TView<Real, 1, D> x0)
      ->std::tuple<TPack<Real, 1, D>, TPack<Vec<Real, 3>, 1, D>,
                   TPack<Vec<Real, 3>, 1, D>, TPack<Vec<Real, 3>, 1, D> > {
    auto num_Vs = parameter_indices.size(0);
...
struct CartBondedTorsionDispatch {
  static auto f(TView<Vec<Int, 4>, 1, D> atomquad_indices,
                TView<Int, 1, D> parameter_indices,
                TView<Vec<Real, 3>, 1, D> coords, TView<Real, 1, D> K,
                TView<Real, 1, D> x0, TView<Int, 1, D> period)
      ->std::tuple<TPack<Real, 1, D>, TPack<Vec<Real, 3>, 1, D>,
                   TPack<Vec<Real, 3>, 1, D>, TPack<Vec<Real, 3>, 1, D>,
                   TPack<Vec<Real, 3>, 1, D> > {
    auto num_Vs = parameter_indices.size(0);
    ...
    auto f_i = ([ = ] EIGEN_DEVICE_FUNC(int i) {
      ...
      tie(Vs[i], dV_dIs[i], dV_dJs[i], dV_dKs[i], dV_dLs[i]) =
          cbtorsion_V_dV(coords[ati], coords[atj], coords[atk], coords[atl],
                         K[pari], x0[pari], period[pari]);
    });
...
struct CartBondedHxlTorsionDispatch {
  static auto f(TView<Vec<Int, 4>, 1, D> atomquad_indices,
                TView<Int, 1, D> parameter_indices,
                TView<Vec<Real, 3>, 1, D> coords, TView<Real, 1, D> K1,
                TView<Real, 1, D> K2, TView<Real, 1, D> K3,
                TView<Real, 1, D> phi1, TView<Real, 1, D> phi2,
                TView<Real, 1, D> phi3)
      ->std::tuple<TPack<Real, 1, D>, TPack<Vec<Real, 3>, 1, D>,
                   TPack<Vec<Real, 3>, 1, D>, TPack<Vec<Real, 3>, 1, D>,
                   TPack<Vec<Real, 3>, 1, D> > {
    ...
    auto f_i = ([ = ] EIGEN_DEVICE_FUNC(int i) {
      ...
      Int pari = parameter_indices[i];
      tie(Vs[i], dV_dIs[i], dV_dJs[i], dV_dKs[i], dV_dLs[i]) =
          cbhxltorsion_V_dV(coords[ati], coords[atj], coords[atk], coords[atl],
                            K1[pari], K2[pari], K3[pari], phi1[pari],
                            phi2[pari], phi3[pari]);
    });
...
template struct CartBondedHxlTorsionDispatch<tmol::Device::CUDA, float,
                                             int32_t>;
template struct CartBondedHxlTorsionDispatch<tmol::Device::CUDA, double,
                                             int32_t>;
