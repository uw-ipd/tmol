...
def test_smoke(default_database, torch_device: torch.device):
    disulfide_energy = DisulfideEnergyTerm(param_db=default_database, device=torch_device)

    assert disulfide_energy.device == torch_device
    assert disulfide_energy.global_params.a_mu.device == torch_device


def test_annotate_disulfide_conns(fresh_default_packed_block_types, default_database, torch_device: torch.device):
    disulfide_energy = DisulfideEnergyTerm(param_db=default_database, device=torch_device)
    ...
    assert pbt.disulfide_conns is disulfide_conns  # Test to make sure the parameters remain the same instance
...
class TestDisulfideEnergyTerm(EnergyTermTestBase):
    ...
    def test_whole_pose_scoring_10(cls, disulfide_pdb, default_database, torch_device):
        return super().test_whole_pose_scoring_10(disulfide_pdb, default_database, torch_device, update_baseline=False)
    ...
    @classmethod
    def test_whole_pose_scoring_gradcheck(cls, disulfide_pdb, default_database, torch_device):
    ...
    @classmethod
    def test_block_scoring_matches_whole_pose_scoring(cls, disulfide_pdb, default_database, torch_device):
        return super().test_block_scoring_matches_whole_pose_scoring(disulfide_pdb, default_database, torch_device)
    ...
    @classmethod
    def test_block_scoring_reweighted_gradcheck(cls, disulfide_pdb, default_database, torch_device):
        resnums = [(2, 4), (21, 23)]
