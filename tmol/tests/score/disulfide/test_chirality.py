...
The fixed fixture goldens use the same published equations and parameter
transport on a stereochemically valid L-Cys/D-Cys derivative of 6DMZ.
"""
...
HETEROCHIRAL_FIXTURE_ENERGY = -0.048889146701669
HETEROCHIRAL_FIXTURE_GRADIENT = [
    [0.102060312346924, -0.092600399961500, -0.038244373611303],
    [-0.395890114169443, -0.190899749054175, 1.451954166735129],
    [0.558988915318207, -3.399978105855607, 0.284000105062732],
    [0.769694691688294, 3.781729149220900, -2.078569436316526],
    [-0.960205509336605, -0.159923409938581, 0.436488847067119],
    [-0.074648295847377, 0.061672515588964, -0.055629308937151],
]


def scoring_atoms(pose):
    atoms = []
...
def pose_for(pair, device, reverse=False, mirror=False):
    ...
    return pose, scoring_atoms(pose)
...
def test_heterochiral_fixture_matches_rosetta_goldens(torch_device):
    array = pdbx.get_structure(
        pdbx.CIFFile.read(data_path("disulfide/6dmz_3_47_ld.cif")),
        model=1,
        include_bonds=True,
    )
    ...
    assert [bt.properties.polymer.sidechain_chirality for bt in block_types] == [
        "l",
        "d",
    ]

    score = ScoreFunction(ParameterDatabase.get_default(), torch_device)
    score.set_weight(ScoreType.disulfide, 1.0)
    coords = pose.coords.double().detach().requires_grad_(True)
    energy = score.render_whole_pose_scoring_module(pose)(coords).sum()
    gradient = torch.autograd.grad(energy, coords)[0][0, scoring_atoms(pose)]

    expected_gradient = torch.tensor(
        HETEROCHIRAL_FIXTURE_GRADIENT, dtype=torch.double, device=torch_device
    )
    torch.testing.assert_close(
        energy,
        torch.tensor(
            HETEROCHIRAL_FIXTURE_ENERGY, dtype=torch.double, device=torch_device
        ),
        atol=3e-6,
        rtol=3e-6,
    )
    torch.testing.assert_close(gradient, expected_gradient, atol=6e-5, rtol=4e-5)


@pytest.mark.parametrize("pair", ["ll", "ld", "dd"])
def test_energy_and_both_native_gradient_paths_match_reference(pair, torch_device):
    ...
    coords[0, atoms[3]] = coords[0, atoms[2]] + 2.05 * direction / direction.norm()
    coords.requires_grad_(True)
    expected = reference(coords[0, atoms], pair, db.scoring.disulfide.global_parameters)
    ...
    torch.testing.assert_close(weighted_grad, expected_grad * 0.7, atol=4e-5, rtol=4e-5)


@pytest.mark.parametrize("pair", ["ll", "ld"])
