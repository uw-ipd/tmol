...
def test_noncanonical_parameter_replay_and_minimization(
    ...
    # The goldens below are absolute energies, so they need geometry that does
    # not depend on where the conformer was embedded. prepare_ligands generates
    # a 3D conformer, and a fixed seed does not make RDKit/OpenBabel embedding
    # bit-identical across architectures -- the two CI arches disagree by far
    # more than SCORE_GOLDEN_ATOL. Score the params committed beside this test,
    # which carry the conformer as icoors; the generated pair above still has
    # to agree with itself, so the generate-and-reload path stays covered.
    pinned, _ = prepare_ligands(
        array,
        params_files=[
            str(data_path("ligand_params", f"noncanonical_{backbone_class}.tmol"))
        ],
    )
    poses, scores, gradients = [], [], []
    for database in (prepared, restored, pinned):
    ...
    if moved:
        # pytest shortens the repr above, so the moved lanes alone are not enough
        # to refresh the file. Emit the whole block: a legitimate change here is
        # rewritten by pasting this over the class entry in the goldens.
        print(
            f"regenerated {backbone_class} goldens:\n"
            + "".join(
                f"  {name}: {value:.4f}\n" for name, value in sorted(by_name.items())
            )
        )
    assert not moved, moved
    _score_and_minimize(poses[-1], context, max_iter=5)
