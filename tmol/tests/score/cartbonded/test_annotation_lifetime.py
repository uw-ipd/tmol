...
from tmol.score._annotation_cache import cached_annotation
...
def test_equal_content_databases_do_not_alias_owner_annotations(
    default_database, torch_device
):
    pose = extended_pose_stack_from_sequences(["AA"], device=torch_device)
    equal_cartbonded = attr.evolve(default_database.scoring.cartbonded)
    equal_database = attr.evolve(
        default_database,
        scoring=attr.evolve(default_database.scoring, cartbonded=equal_cartbonded),
    )
    terms = [
        CartBondedEnergyTerm(database, torch_device)
        for database in (default_database, equal_database)
    ]

    first = terms[0].setup_packed_block_types(pose.packed_block_types)
    second = terms[1].setup_packed_block_types(pose.packed_block_types)

    assert terms[0].hash == terms[1].hash
    assert first is not second
    assert terms[0].setup_packed_block_types(pose.packed_block_types) is first
    assert len(pose.packed_block_types._cartbonded_annotation) == 2
...
def test_eviction_bounds_annotations_and_preserves_rendered_modules(
    ...
    assert all(len(bt._cartbonded_annotation) <= 2 for bt in pbt.active_block_types)
    assert (
        cached_annotation(
            pbt, "_cartbonded_annotation", terms[0]._packed_annotation_key
        )
        is None
    )
