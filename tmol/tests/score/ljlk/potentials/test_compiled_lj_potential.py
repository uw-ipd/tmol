...
def test_lj_gradcheck(params, iname, jname, bonded_path_length):
    ...
    gradcheck(
        lambda dist: compiled.LJScore.apply(dist, bonded_path_length, i_params, j_params, global_params).sum(),
...
def test_lj_spotcheck(params, iname, jname):
    ...
    def eval_lj(dist, bonded_path_length=5):
        dist = dist.clone().detach() if isinstance(dist, torch.Tensor) else torch.tensor(dist)

        return compiled.LJScore.apply(dist.requires_grad_(True), bonded_path_length, i, j, g).detach().numpy()

    def eval_lj_alone(dist, bonded_path_length=5):
        dist = dist.clone().detach() if isinstance(dist, torch.Tensor) else torch.tensor(dist)

        return compiled.LJScore.apply(dist.requires_grad_(False), bonded_path_length, i, j, g).numpy()

    def eval_d_lj_d_dist(dist, bonded_path_length=5):
        dist = dist.clone().detach() if isinstance(dist, torch.Tensor) else torch.tensor(dist)

        compiled.LJScore.apply(dist.requires_grad_(True), bonded_path_length, i, j, g).sum().backward()

        return dist.grad.numpy()

    # Linear region
    assert eval_lj(0.6 * sigma - 1.0) == approx(eval_lj(0.6 * sigma) - eval_d_lj_d_dist(0.6 * sigma))
    assert eval_d_lj_d_dist(numpy.linspace(0, 0.6 * sigma)) == approx(float(eval_d_lj_d_dist(0.6 * sigma)))

    # Minimum value at sigma
    assert eval_lj(sigma) == approx(-epsilon)
    assert eval_lj(numpy.linspace(0, 8, 1000)).min() == approx(-epsilon, abs=1e-5, rel=0)
