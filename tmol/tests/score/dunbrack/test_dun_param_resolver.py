...
def test_dun_param_resolver_construction(default_database, torch_device):
    ...
    assert len(dun_params.rotameric_prob_tables) == nlibs
    assert len(dun_params.rotameric_neglnprob_tables) == nlibs
    assert len(dun_params.rotameric_mean_tables) == nlibs
    assert len(dun_params.rotameric_sdev_tables) == nlibs
    ...
    for i, rotlib in enumerate(
        ...
        assert (
            rotlib.rotameric_data.rotamers.shape[0]
            == dun_params.rotameric_prob_tables[i].shape[0]
        )
    ...
    for i, rotlib in enumerate(
        ...
        assert (
            rotlib.rotameric_data.rotamers.shape[0]
            * rotlib.rotameric_data.rotamers.shape[1]
            == dun_params.rotameric_mean_tables[i].shape[0]
        )
    ...
    assert len(dun_params.semirotameric_tables) == nsemirotameric_libs

    for i, rotlib in enumerate(dunlib.semi_rotameric_libraries):
        assert (
            rotlib.rotameric_chi_rotamers.shape[0]
            == dun_params.semirotameric_tables[i].shape[0]
        )
    ...
    # gone rotprob_offsets = dun_params.rotameric_prob_tableset_offsets.cpu().numpy()
    ...
    # gone rotmean_offsets = dun_params.rotameric_meansdev_tableset_offsets.cpu().numpy()
    # gone rotsandchi = [
    ...
    print("nrotamers", [rotamer.shape[0] for rotamer in rotamers])
    print(rotameric_ri2ti)
    ...
    for i in range(rotameric_ri2ti_offsets.shape[0]):
        ...
        for j in range(rotamers[i].shape[0]):
            ...
            print(
                ...
                j,
                rotind,
                rotamers[i].numpy()[j, :] - 1,
                n_nonrot_bins,
                rotameric_ri2ti[rotind],
                tableind,
            )
            assert rotameric_ri2ti[rotind] == tableind

    # gone semirot_offsets = dun_params.semirotameric_tableset_offsets.cpu().numpy()
