...
def test_smoke(default_database, torch_device):
    backbone_torsion_energy = BackboneTorsionEnergyTerm(param_db=default_database, device=torch_device)
...
def test_whole_pose_scoring_module_smoke(ubq_pdb, default_database, torch_device):
    ...
    backbone_torsion_pose_scorer = backbone_torsion_energy.render_whole_pose_scoring_module(p1)

    coords = torch.nn.Parameter(p1.coords.clone())
    scores = backbone_torsion_pose_scorer(coords)

    # make sure we're still good
    numpy.testing.assert_allclose(gold_vals, scores.cpu().detach().numpy(), atol=1e-3, rtol=0)
...
class TestBackboneTorsionEnergyTerm(EnergyTermTestBase):
    ...
    def test_block_scoring_reweighted_gradcheck(cls, ubq_pdb, default_database, torch_device):
        resnums = [(6, 12)]
        return super().test_block_scoring_reweighted_gradcheck(ubq_pdb, default_database, torch_device, resnums=resnums)
