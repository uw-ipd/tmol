...
class TestGroundTruthRegression:
    """Validate pipeline against reference outputs.

    Compares all deterministic pipeline outputs (atom types, charges,
    bond topology, chi definitions, NBR_ATOM, ICOOR tree structure,
    and protonation) against reference .params files from a validated
    ligand_prep pipeline run.
    """
    ...
    def test_nbr_atom_matches(self, ref_data):
        """NBR_ATOM (neighbor/root atom) must match reference."""
        ref_nbr = ref_data["ref"]["nbr_atom"]
        actual_nbr = ref_data["restype"].default_jump_connection_atom
        assert actual_nbr == ref_nbr, (
            f"NBR_ATOM mismatch for {ref_data['name']}: "
            f"got {actual_nbr!r}, expected {ref_nbr!r}"
        )

    def test_icoor_topology_matches(self, ref_data):
        """ICOOR parent/grandparent/great-grandparent names must match.

        The atom tree topology is deterministic; only d/theta/phi values
        depend on 3D coordinates.
        """
        ref_topo = ref_data["ref"]["icoor_topology"]
        actual_icoors = ref_data["restype"].icoors

        actual_topo = {}
        for ic in actual_icoors:
            actual_topo[ic.name] = (
                ic.parent,
                ic.grand_parent,
                ic.great_grand_parent,
            )

        assert set(actual_topo.keys()) == set(ref_topo.keys()), (
            f"ICOOR atom set mismatch for {ref_data['name']}:\n"
            f"  missing: {set(ref_topo.keys()) - set(actual_topo.keys())}\n"
            f"  extra:   {set(actual_topo.keys()) - set(ref_topo.keys())}"
        )

        for atom_name in ref_topo:
            assert actual_topo[atom_name] == ref_topo[atom_name], (
                f"ICOOR topology mismatch for {atom_name} "
                f"in {ref_data['name']}: "
                f"got {actual_topo[atom_name]}, "
                f"expected {ref_topo[atom_name]}"
            )
