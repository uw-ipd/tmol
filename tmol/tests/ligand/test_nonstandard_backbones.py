...
def _heavy_only(atom_array):
    """Drop explicitly modeled hydrogens while preserving heavy topology."""
    return atom_array[~numpy.isin(atom_array.element, ("H", "D"))]
...
def test_heavy_only_terminal_sar_is_a_polymer_but_b3k_stays_ambiguous() -> None:
    ...
    profile = profile_for_atom_array(sar, frozenset({"C"}))
    assert profile is not None
    assert profile.mainchain_atoms == ("N", "CA", "C")

    b3k = _heavy_only(_residue_bonded_only_at("B3K", "C"))
    assert profile_for_atom_array(b3k, frozenset({"C"})) is None


def test_unsupported_heavy_only_nitrogen_is_not_a_cap() -> None:
    """A multiply bonded nitrogen is unknown, not evidence of saturation."""
    residue = _heavy_only(_residue_bonded_only_at("SAR", "C"))
    indices = {str(name): i for i, name in enumerate(residue.atom_name)}
    bonds = residue.bonds.as_array()
    edge = numpy.all(
        numpy.sort(bonds[:, :2], axis=1) == sorted((indices["N"], indices["CN"])),
        axis=1,
    )
    bonds[edge, 2] = int(struc.BondType.DOUBLE)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("FGA", frozenset({"N", "CD"}), ("N", "CA", "CB", "CG", "CD")),
    ],
)
def test_heavy_only_internal_beta_and_gamma_use_observed_endpoints(
    code: str, connections: frozenset, expected: tuple[str, ...]
) -> None:
    """Observed internal connections define non-alpha backbone paths."""
    profile = profile_for_atom_array(_heavy_only(_residue(code)), connections)
    assert profile is not None
    assert profile.mainchain_atoms == expected
...
def test_b3k_endpoint_inference_uses_complete_template_before_restoration(
    monkeypatch,
) -> None:
    """A missing sidechain amine remains an ambiguity after heavy restoration."""
    from tmol.io import _cif
    from tmol.ligand import _polymer_profile

    template = info.residue("B3K")
    template.res_name[:] = "B3K"
    residue = _heavy_only(_residue_bonded_only_at("B3K", "C"))
    residue = residue[residue.atom_name != "NZ"]
    partner = struc.AtomArray(1)
    partner.atom_name[:] = "X"
    partner.element[:] = "C"
    partner.res_name[:] = "LNK"
    partner.chain_id[:] = residue.chain_id[0]
    partner.res_id[:] = int(residue.res_id[0]) + 1
    partner.coord[:] = residue.coord[0]
    structure = struc.concatenate([residue, partner])
    carbonyl = int(numpy.flatnonzero(structure.atom_name == "C")[0])
    structure.bonds.add_bond(carbonyl, len(residue), struc.BondType.SINGLE)

    inferred_from = []
    completed_connection_atoms = _polymer_profile.completed_connection_atoms

    def record_inference_source(atom_array, connection_atoms):
        inferred_from.append(frozenset(atom_array.atom_name))
        return completed_connection_atoms(atom_array, connection_atoms)
    ...
    assert "NZ" in set(b3k.atom_name)
    assert completed_connection_atoms(b3k, frozenset({"C"})) == frozenset({"C"})
...
def test_declared_chemistry_completes_an_unresolved_sidechain() -> None:
    ...
    assert _prepared_beta_peptide(structure)[code] == (n_atoms, n_chi)
    # A source-defined alias must use its own geometry, even when the bundled
    # CCD assigns that same code to another molecule.
    renamed = structure.copy()
    renamed.res_name[renamed.res_name == code] = "X3K"
    template = renamed._custom_ccd_registry[code].copy()
    template.res_name[:] = "X3K"
    renamed._custom_ccd_registry = {**renamed._custom_ccd_registry, "X3K": template}
    missing = np.isnan(renamed.coord).copy()
    assert _prepared_beta_peptide(renamed)["X3K"] == (n_atoms, n_chi)
    np.testing.assert_array_equal(np.isnan(renamed.coord), missing)


def test_declared_geometry_requires_observed_placement_anchors() -> None:
    """Complete a partial residue, but reject one with no resolved atoms."""
    from tmol.ligand._detect import with_resolved_coordinates

    code = _TRUNCATED[0]
    template = _structure("beta_peptide_3c3g")._custom_ccd_registry[code].copy()
...
def test_a_residue_nothing_describes_is_assumed_complete() -> None:
    """Preserve supplied chemistry for preparation when CCD has no definition."""
    from tmol.io._cif import with_unresolved_atoms
    from tmol.ligand import prepare_ligands
    ...
    numpy.testing.assert_array_equal(
        structure.bonds.as_array(), supplied.bonds.as_array()
    )
...
def test_a_params_file_carries_the_patches_it_needs(tmp_path) -> None:
    ...
    write_params_file([prep], str(path))
