...
def prepare_uncorrected_conjugate(array, **kwargs):
    ...
    with pytest.MonkeyPatch.context() as patch:
        patch.setattr(
            ...
            lambda array, db, ph, strict_ligands=True: (db, (), ()),
        )
...
def _serine_bonded_to_boronate():
    """A serine whose OG carries a boronic acid: tmol has no boron atom type."""
    from biotite.structure.info import residue

    serine = residue("SER")
    serine = serine[serine.element != "H"]
    serine.chain_id[:] = "A"
    serine.res_id[:] = 1

    boronate = serine[:3].copy()
    boronate.atom_name = np.array(["B", "O1", "O2"])
    boronate.element = np.array(["B", "O", "O"])
    boronate.res_name[:] = "BXX"
    boronate.res_id[:] = 2
    boronate.hetero[:] = True
    boronate.coord = serine.coord[serine.atom_name == "OG"] + np.array(
        [[1.5, 0.0, 0.0], [2.2, 1.2, 0.0], [2.2, -1.2, 0.0]]
    )
    boronate.bonds = struc.BondList(3, np.array([[0, 1, 1], [0, 2, 1]]))

    array = serine + boronate
    og = int(np.flatnonzero(array.atom_name == "OG")[0])
    boron = int(np.flatnonzero(array.element == "B")[0])
    array.bonds.add_bond(og, boron, struc.BondType.SINGLE)
    return array


def test_unprepared_covalent_partner_is_refused_or_cut_but_never_a_crash():
    """A bonded residue preparation skipped must not escape as a bare ValueError.

    ``_ligand_unsupported_reason`` skips a boron ligand whatever the flag says,
    and the residue stays covalently bonded in the input. The conjugate model
    refuses to guess connectivity for a residue it has no definition for, so
    strict preparation has to own that refusal and ``strict_ligands=False`` has
    to honour its own contract and drop the link.
    """
    from tmol.database import ParameterDatabase
    from tmol.ligand._preparation import (
        LigandPreparationError,
        _prepare_conjugate_params,
    )

    array = _serine_bonded_to_boronate()
    param_db = ParameterDatabase.get_default()

    with pytest.raises(LigandPreparationError, match="covalently bonded but not"):
        _prepare_conjugate_params(array, param_db, 7.4, strict_ligands=True)

    prepared, records, replacements = _prepare_conjugate_params(
        array, param_db, 7.4, strict_ligands=False
    )
    assert prepared is param_db and not records and not replacements


def test_cutting_an_unprepared_partner_keeps_its_own_bonds():
    """Only the bond across residues goes; the ligand's own chemistry is intact."""
    array = _serine_bonded_to_boronate()
    trimmed = without_cross_bonds_to(array, {"BXX"})

    def pairs(structure):
        return {frozenset(bond[:2]) for bond in structure.bonds.as_array().tolist()}
