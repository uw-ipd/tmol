...
def test_real_preparation_maps_roundtrip_and_strict_injection(prep, tmp_path):
    ...
    assert yaml.safe_load(path.read_text())["version"] == TMOL_FORMAT_VERSION
...
def test_invalid_element_metadata_is_rejected(prep, tmp_path, mapping):
    ...
    write_params_file(replace(prep, atom_type_elements=None), path)
...
def test_unreadable_format_version_is_rejected(prep, tmp_path):
    """Writers always stamp the current version, so an unknown major is a file
    this reader cannot claim to understand."""
    path = tmp_path / "old.tmol"
    write_params_file(prep, path)
    raw = yaml.safe_load(path.read_text())
    raw["version"] = "99.0"
    path.write_text(yaml.safe_dump(raw))
    with pytest.raises(ValueError, match="incompatible with the current format"):
        load_params_file(path)
