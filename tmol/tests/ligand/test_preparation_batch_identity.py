...
def test_repeated_files_and_repeated_export_have_one_identity(prep, tmp_path):
    ...
    write_params_file(prep, one)
    write_params_file([prep, replace(prep)], many)
...
def test_each_path_is_read_once_per_batch(prep, tmp_path, monkeypatch):
    ...
    assert calls == [path]
    # Each batch still calls the reader; its metadata cache owns parse reuse.
    inject_params_files(ParameterDatabase.get_default(), [path])
    assert calls == [path, path]

    # Cached prototypes must not leak mutable charge edits to a later caller.
    loaded = original(path)
    atom = next(iter(loaded[0].partial_charges))
    charge = loaded[0].partial_charges[atom]
    loaded[0].partial_charges[atom] += 1.0
    assert original(path)[0].partial_charges[atom] == charge
