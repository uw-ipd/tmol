...
def test_plp_enzyme_completion_preserves_residues_and_terminal_chemistry(
    ...
    def capture(*args, **kwargs):
        ...
        types = current.packed_block_types.active_block_types
        llp_blocks = [
            i
            for i, t in enumerate(current.block_type_ind[0].tolist())
            if t >= 0 and types[t].base_name == "LLP"
        ]
        assert llp_blocks
        counts.extend(rotamers.n_rots_for_block[0, llp_blocks].tolist())
        assert all(1 < count <= 1001 for count in counts)
        return current, rotamers

    monkeypatch.setattr(packing, "build_rotamers", capture)
    score = beta2016_score_function(torch_device, param_db=db)
