...
def test_ptm_biotite_round_trip_and_scoring(torch_device, base, variant, name3):
    ...
    # Built-in modified residues remain known when the mixed protein/ligand
    # preparation path is enabled; they must not be registered as new ligands.
    round_trip = pose_stack_from_biotite(structure, torch_device, prepare_ligands=True)
