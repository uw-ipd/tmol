...
def test_canonical_form_from_ubq_pose(ubq_pdb, torch_device):
    ...
    (cf_orig_chain_id, cf_orig_res_types, cf_orig_coords, cf_orig_chain_labels) = tuple(
        (
            canonical_form[x].clone()
            if isinstance(canonical_form[x], torch.Tensor)
            else canonical_form[x].copy()
        )
...
def test_canonical_form_from_jagged_ubq_pose(ubq_pdb, torch_device):
    ...
    (
        ...
        (
            canonical_form4[x].clone()
            if isinstance(canonical_form4[x], torch.Tensor)
            else canonical_form4[x].copy()
        )
        for x in ["chain_id", "res_types", "coords", "chain_labels"]
    )
    ...
    (
        ...
        (
            canonical_form6[x].clone()
            if isinstance(canonical_form6[x], torch.Tensor)
            else canonical_form6[x].copy()
        )
        for x in ["chain_id", "res_types", "coords", "chain_labels"]
    )
