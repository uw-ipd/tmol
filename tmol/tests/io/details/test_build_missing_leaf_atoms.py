...
def ats_to_not_rebuild(pbt, torch_device):
    ...
    ats_in_bts_to_leave_alone = torch.ones((pbt.n_types, pbt.max_n_atoms), dtype=torch.bool, device=torch_device)
    for i, bt in enumerate(pbt.active_block_types):
        ...
        if bt.base_name in ats_generally_to_leave_be:
            ats_in_bts_to_leave_alone[i, bt.atom_to_idx[ats_generally_to_leave_be[bt.base_name]]] = False
...
def test_build_missing_leaf_atoms(torch_device, ubq_pdb):
    ...
    block_coords, missing_atoms, real_atoms, _, _occ, _b_factor = take_block_type_atoms_from_canonical(
    ...
    block_at_is_leaf = torch.zeros((n_poses, max_n_blocks, pbt.max_n_atoms), dtype=torch.bool, device=torch_device)
    real_blocks = block_types64 >= 0
    block_at_is_leaf[real_blocks] = pbt.is_leaf_atom[block_types64[real_blocks]].to(torch.bool)
    ats_in_bts_to_leave_alone = ats_to_not_rebuild(pbt, torch_device)
    block_at_allow_rebuild = torch.ones((n_poses, max_n_blocks, pbt.max_n_atoms), dtype=torch.bool, device=torch_device)
    block_at_allow_rebuild[real_blocks] = ats_in_bts_to_leave_alone[block_types64[real_blocks]]
    ...
    new_pose_coords, block_coord_offset, real_block_atoms, pose_at_is_real = build_missing_leaf_atoms(
        pbt,
        block_types64,
        real_atoms,
        block_coords,
    ...
    n_ats_per_block = torch.zeros((n_poses, max_n_blocks), dtype=torch.int32, device=torch_device)
    n_ats_per_block[real_blocks] = pbt.n_atoms[block_types64[real_blocks]]
    real_expanded_pose_ats = n_ats_per_block_arange_expanded < n_ats_per_block.unsqueeze(2)
...
def test_build_missing_leaf_atoms_error_handling(torch_device, ubq_pdb):
    ...
    block_coords, missing_atoms, real_atoms, _, _occ, _bf = take_block_type_atoms_from_canonical(
        pbt, block_types64, coords, at_is_pres, cf.atom_occupancy, cf.atom_b_factor
    )
...
def test_build_missing_leaf_atoms_backwards(torch_device, ubq_pdb):
    ...
    class FauxModule(torch.nn.Module):
        ...
        def forward(self):
            found_disulfides, res_type_variants = find_disulfides(co, can_rts, self.coords)
            # 3
            (
                his_taut,
                res_type_variants,
                resolved_coords,
                resolved_atom_is_present,
            ) = resolve_his_tautomerization(co, can_rts, res_type_variants, self.coords, at_is_pres)
...
def test_coord_sum_gradcheck(torch_device, ubq_pdb):
    ...
    res_not_connected = torch.tensor([[[False, True]]], dtype=torch.bool, device=torch_device)
