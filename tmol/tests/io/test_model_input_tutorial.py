...
def test_model_specific_constructors_are_not_public():
    import tmol
    import tmol.io

    for name in ("pose_stack_from_openfold", "pose_stack_from_rosettafold2"):
        assert not hasattr(tmol, name)
        assert not hasattr(tmol.io, name)
