...
def test_cart_minimize_w_pose_and_sfxn_smoke(ubq_pdb, default_database, torch_device):
    ...
    E0 = cart_sfxn_network.whole_pose_scoring_module(cart_sfxn_network.full_coords).sum()
    ...
    E1 = cart_sfxn_network.whole_pose_scoring_module(cart_sfxn_network.full_coords).sum()
    assert E1 < E0
...
def test_kin_minimize_w_pose_and_sfxn_smoke(ubq_pdb, default_database, torch_device):
    ...
    kin_module = PoseStackKinematicsModule(pose_stack5, FoldForest.polymeric_forest(numpy.full(5, n_res)))
...
def test_minimize_w_pose_and_sfxn_benchmark(benchmark, ubq_pdb, default_database, torch_device, n_poses):
...
def test_profile_minimizer(ubq_pdb, torch_device):
    ...
    sfxn = beta2016_score_function(torch_device)
    ff = FoldForest.polymeric_forest(numpy.full(N_poses, pose_stackN.max_n_blocks, dtype=numpy.int32))
...
def test_profile_minimizer2(erbb2_and_pertuzumab_pdb, torch_device):
    ...
    if torch_device == torch.device("cpu"):
        return
    pose_stack1 = pose_stack_from_pdb(erbb2_and_pertuzumab_pdb, torch_device, res_not_connected=res_not_connected)
    pose_stackN = PoseStackBuilder.from_poses([pose_stack1] * N_poses, torch_device)
