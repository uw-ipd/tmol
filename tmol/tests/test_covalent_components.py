...
@pytest.mark.parametrize("fixture", sorted(FIXTURES))
def test_attachment_matrix_h_geometry_score_and_gradient(fixture, torch_device):
    """Check glycan/ligand attachment H geometry through differentiable scoring."""
    stem = FIXTURES[fixture]
    atom_array = structure(stem)
    ...
    residue, site = next(iter(CANONICAL_SITES[stem].items()))
    used_types = {
        int(index)
        for index in pose.block_type_ind.flatten().tolist()
        if int(index) >= 0
    }
    block_type = next(
        pose.packed_block_types.active_block_types[index]
        for index in used_types
        if pose.packed_block_types.active_block_types[index].base_name == residue
        and any(
            connection.name == f"conj_{site}"
            for connection in pose.packed_block_types.active_block_types[
                index
            ].connections
        )
    )
    hydrogens = _hydrogens_on(block_type, site, context.parameter_database.chemical)
    assert len(hydrogens) == EXPECTED_RETAINED_H[stem]

    ideal = block_type.ideal_coords[block_type.at_to_icoor_ind]
    for hydrogen in hydrogens:
        h_index = block_type.atom_to_idx[hydrogen]
        parent_index = block_type.atom_to_idx[site]
        assert any(
            {first, second} == {hydrogen, site}
            for first, second, *_ in block_type.bonds
        )
        assert (
            0.85
            < float(((ideal[h_index] - ideal[parent_index]) ** 2).sum() ** 0.5)
            < 1.15
        )
    ...
    values = score(coords, sum_terms=False, apply_weights=False)
    (gradient,) = torch.autograd.grad(values.sum(), coords)
    assert torch.isfinite(values).all()
    assert torch.isfinite(gradient[pose.real_atoms]).all()
    assert torch.count_nonzero(gradient[pose.real_atoms]) > 0
...
def test_retyping_leaves_the_unconjugated_residue_alone():
    ...
    attachment = next(a for a in by_name["ASN:conj_ND2"].atoms if a.name == "ND2")
    assert attachment.genbonded_type == "Nad"
