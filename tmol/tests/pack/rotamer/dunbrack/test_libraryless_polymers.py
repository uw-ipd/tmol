...
def test_own_chi_count_obeys_explicit_budget(ubq_pdb, default_database, torch_device):
    ...
    concrete = SetPackerTask.from_packer_task(task)
    counts, _, _, _ = sampler.sample_chi_for_poses(pose, concrete)
    assert int(counts.max()) == 1
    _, rotamers = build_rotamers(pose, concrete, default_database.chemical)
    targets = torch.isin(rotamers.block_type_ind_for_rot, target_types(pose))
    for rot in torch.nonzero(targets).flatten().tolist():
        block = int(rotamers.block_ind_for_rot[rot])
        bt = pose.packed_block_types.active_block_types[
            int(pose.block_type_ind[0, block])
        ]
        first = int(rotamers.coord_offset_for_rot[rot])
        original = int(pose.block_coord_offset[0, block])
        for sample in samples:
            atoms = [u[0] for u in bt.torsion_to_uaids[sample.chi_dihedral]]
            actual = coord_dihedrals(
                *rotamers.coords[first + torch.tensor(atoms, device=torch_device)][
                    :, None
                ].double()
            )
            expected = coord_dihedrals(
                *pose.coords[0, original + torch.tensor(atoms, device=torch_device)][
                    :, None
                ].double()
            )
            delta = torch.atan2(
                torch.sin(actual - expected), torch.cos(actual - expected)
            )
            torch.testing.assert_close(
                delta, torch.zeros_like(delta), atol=2e-5, rtol=0
            )
