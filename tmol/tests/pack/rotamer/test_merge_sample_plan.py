...
@pytest.mark.parametrize(
    "counts, indices, dtype, match",
    [
        # counts that are not a plan at all
        ([-1, 2], [1], torch.int32, "sample counts"),
        ([1, 2], [1], torch.int32, "sample counts"),
        # a sum that would wrap int64 into an empty plan
        ([2**62] * 4, [], torch.int64, "sample counts"),
        # source rows that do not match the ordered count intervals
        ([1, 1], [1, 0], torch.int32, "source rows.*ordered sample counts"),
        ([2, 1], [0, 1, 1], torch.int32, "source rows.*ordered sample counts"),
        ([1, 1], [-1, 1], torch.int32, "source rows.*ordered sample counts"),
        ([1, 1], [0, 2], torch.int32, "source rows.*ordered sample counts"),
        ([3, 2], [0, 1, 0, 1, 1], torch.int32, "source rows.*ordered sample counts"),
    ],
)
def test_merge_rejects_a_plan_it_cannot_scatter(
    counts, indices, dtype, match, torch_device
):
    """Every malformed plan is refused before anything is written."""
    sample = (
        torch.tensor(counts, dtype=dtype, device=torch_device),
