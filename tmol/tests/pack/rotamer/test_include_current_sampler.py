...
def test_include_current_sampler_smoke(ubq_pdb, torch_device, default_restype_set):
    ...
    for i, (pose, res) in enumerate(residues_to_fix):
        ...
        curr_rt_in_considered = task.blts[pose][res].considered_block_types.index(curr_rt)
        i_gbt = (pose * 6 + res) * 21 + curr_rt_in_considered
        n_rots_for_rt_gold[i_gbt] = 1  # 6 cause p0 has 6 res and we only have two poses to worry about
