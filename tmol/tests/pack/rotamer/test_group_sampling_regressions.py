...
from tmol.pack.rotamer._conjugated_groups import group_sampled_chi
...
def test_group_budget_keeps_correct_residue_and_linkage(monkeypatch):
    ...
    rkd = SimpleNamespace(bfto_2_orig=np.array([0, 2, 1]), preds=np.array([-1, 2, 0]))
    monkeypatch.setattr(
        tmol.kinematics,
...
def test_kinforest_cache_is_scoped_to_packed_block_types(monkeypatch):
    ...
    def construct(types, links, anchor):
