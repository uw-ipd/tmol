...
def test_a_group_is_sampled_as_the_product_of_its_parts(torch_device):
    ...
    for group, columns, conformers in enumerated:
        ...
        n_tree = 1
        grid = conformers[:-1]
        for column, (owner, _name, _b, _c) in enumerate(columns):
            if owner != 0:
                # Budgeting may remove expansions from the residue type's
                # declared samples before constructing the retained product.
                n_tree *= numpy.unique(grid[:, column]).size
        # the grid is a product of the per-chi sample counts, plus the one
        #    conformer the group came in with
        assert grid.shape[0] % n_tree == 0
...
def _pack_and_check_score(pose_stack, param_db, torch_device, task=None):
    ...
    # CUDA atomic reductions need a double-precision reference when large
    # positive/negative terms cancel; CPU reductions are deterministic.
    coords = new_pose_stack.coords
...
@pytest.mark.parametrize("chemistry", ["biotin", "dna", "peptide_inhibitor_7zv5"])
def test_default_relax_samples_general_chemistry(
    monkeypatch, torch_device, chemistry, dna_pdb, default_database
):
    """Default FastRelax samples attached groups and nucleic acids coherently."""
    import tmol.pack._pack_rotamers as packing
    ...
    if chemistry != "dna":
        pose, context = _pose(FIXTURES.get(chemistry, chemistry), torch_device)
        database = context.parameter_database
    else:
        from tmol.io import pose_stack_from_pdb

        pose = pose_stack_from_pdb(dna_pdb, torch_device)
        database = default_database
    score = beta2016_score_function(torch_device, param_db=database)
    groups = find_conjugated_groups(pose)
    initial = _linkage_lengths(pose, groups)
    build = packing.build_rotamers
    sampled = []

    def capture(*args, **kwargs):
        current, rotamers = build(*args, **kwargs)
        if groups:
            assert rotamers.correlated_groups
        else:
            assert torch.all(rotamers.n_rots_for_block[pose.block_type_ind >= 0] > 1)
        for group in groups:
            counts = rotamers.n_rots_for_block[group.pose, list(group.blocks)]
            assert torch.all(counts == counts[0])
            assert int(counts[0]) >= (1 if chemistry == "peptide_inhibitor_7zv5" else 2)
        sampled.append(True)
        return current, rotamers

    monkeypatch.setattr(packing, "build_rotamers", capture)

    def minimize(current, sfxn, **kwargs):
        for link, length in _linkage_lengths(current, groups).items():
            assert abs(length - initial[link]) < 1e-4
        return run_cart_min(current, sfxn, optimizer_kwargs={"max_iter": 25})
    ...
    for length in _linkage_lengths(relaxed, groups).values():
        assert 1.1 < length < 1.8
