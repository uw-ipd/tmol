...
class TestReactiveProperties(unittest.TestCase):
    def test_cached(self):
        ...
        self.assertCountEqual(
            tp._backend.keys(), ["raw_text", "norm_text", "word_counts"]
        )
    ...
    def test_derived(self):
        class TProp(properties.HasProperties):
            ...
            @derived_from(
                "norm_text", properties.Dictionary("Normalized word counts")
            )
            def word_counts(self):
                return Counter(self.norm_text.split())
        ...
        self.assertEqual(
            len(get_in(["raw_text", "observe_set"], tp._listeners)), 1
        )
        self.assertEqual(
            len(get_in(["norm_text", "observe_set"], tp._listeners)), 1
        )
