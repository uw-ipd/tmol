...
def test_stretch_i32(torch_device):
    ...
    t2 = stretch(t, 3)
    t2_gold = torch.tensor([0, 0, 0, 1, 1, 1, 2, 2, 2, 3, 3, 3], dtype=torch.int32, device=torch_device)
    torch.testing.assert_close(t2_gold, t2)
    assert t2.device == torch_device


def test_stretch2_i32(torch_device):
    t = torch.tensor([[0, 1, 2, 3], [4, 5, 6, 7]], dtype=torch.int32, device=torch_device)
    t2 = stretch2(t, 3)
...
def test_nplus1d_tensor_from_list():
    ...
    for i in range(4):
        for j in range(5):
            for k in range(5):
                assert joined[i, j, k] == ((i + 1) if (j < gold_sizes[i, 0] and k < gold_sizes[i, 1]) else 0)
