...
def test_residue_defs(default_database: tmol.database.ParameterDatabase):
    ...
    assert len(default_database.chemical.atom_types) == len(atom_types), "Duplicate atom types."

    elements = {el.name: el.atomic_number for el in default_database.chemical.element_types}
    ...
    for r in default_database.chemical.residues:
        ...
        assert len(connection_names) == len(r.connections), "connection names not unique."

        # Check that atom and icoor names are unique
        valid_icoor_names = set.union(atom_names, connection_names)
        assert len(valid_icoor_names) == len(r.atoms) + len(r.connections), "atom/icoor name collision"
        ...
        for t in r.torsions:
            for catom in (t.a, t.b, t.c, t.d):
                assert (catom.atom in atom_names and catom.connection is None and catom.bond_sep_from_conn is None) or (
                    catom.atom is None and catom.connection in connection_names and catom.bond_sep_from_conn is not None
                )
