...
def test_hbond_param_import(rosetta_database):
    params = RosettaHBParams(os.path.join(rosetta_database, "scoring/score_functions/hbonds/sp2_elec_params"))
...
def test_rama_table_read(rosetta_database, default_database):
    r3_rama_dir = os.path.join(rosetta_database, "scoring/score_functions/rama/fd_beta_nov2016/")
    r3_paapp_dir = os.path.join(rosetta_database, "scoring/score_functions/P_AA_pp/shapovalov/10deg/kappa131/")
    r3_paa_dir = os.path.join(rosetta_database, "scoring/score_functions/P_AA_pp/")

    r3_general, r3_prepro = rewrite_rama_binary.parse_all_tables(0.5, r3_rama_dir, 0.61, r3_paapp_dir, r3_paa_dir)
    ...
    for aa in r3_general:
        ...
        numpy.testing.assert_allclose(ramatables[gen_idx].bbstep, numpy.array([numpy.pi / 18.0, numpy.pi / 18.0]))
        numpy.testing.assert_allclose(ramatables[gen_idx].bbstart, numpy.array([-numpy.pi, -numpy.pi]))
        ...
        numpy.testing.assert_allclose(ramatables[prepro_idx].bbstart, numpy.array([-numpy.pi, -numpy.pi]))
...
def test_bbdep_omega_table_read(rosetta_database):
    ...
    tables = rewrite_omega_bbdep_binary.parse_all_tables(r3_bbdepomega_dir)
...
def test_dunbrack_table_read(rosetta_database, default_database):
    ...
    fresh = create_dunbrack_rotamer_library(r3_beta16_dunbrack_dir, r3_beta15_dunbrack_dir)
    ...
    def compare_rotameric_data(default_rotdat, fresh_rotdat):
        ...
        torch.testing.assert_close(default_rotdat.rotamer_probabilities, fresh_rotdat.rotamer_probabilities)
        torch.testing.assert_close(default_rotdat.rotamer_means, fresh_rotdat.rotamer_means)
        torch.testing.assert_close(default_rotdat.backbone_dihedral_start, fresh_rotdat.backbone_dihedral_start)
        torch.testing.assert_close(default_rotdat.backbone_dihedral_step, fresh_rotdat.backbone_dihedral_step)
        torch.testing.assert_close(default_rotdat.rotamer_alias, fresh_rotdat.rotamer_alias)

    for i in range(len(default.rotameric_libraries)):
        assert default.rotameric_libraries[i].table_name == fresh.rotameric_libraries[i].table_name
    ...
    def compare_semirotameric_data(default_srdat, fresh_srdat):
        ...
        torch.testing.assert_close(default_srdat.non_rot_chi_start, fresh_srdat.non_rot_chi_start)
        torch.testing.assert_close(default_srdat.non_rot_chi_step, fresh_srdat.non_rot_chi_step)
        torch.testing.assert_close(default_srdat.non_rot_chi_period, fresh_srdat.non_rot_chi_period)
        torch.testing.assert_close(default_srdat.rotameric_chi_rotamers, fresh_srdat.rotameric_chi_rotamers)
        torch.testing.assert_close(
            default_srdat.nonrotameric_chi_probabilities,
            fresh_srdat.nonrotameric_chi_probabilities,
        )
        torch.testing.assert_close(default_srdat.rotamer_boundaries, fresh_srdat.rotamer_boundaries)

    for i in range(len(default.semi_rotameric_libraries)):
        assert default.semi_rotameric_libraries[i].table_name == fresh.semi_rotameric_libraries[i].table_name
        compare_semirotameric_data(default.semi_rotameric_libraries[i], fresh.semi_rotameric_libraries[i])
    os.remove("test_dunbrack.bin")
