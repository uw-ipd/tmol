...
def add_capped_junction_parameters(
    params, capped_params, profile, kept, renamed, references, elements
):
    """Keep generated cap bond/angle ideals where no peptide-role fallback exists.

    The cap stands in for the neighboring peptide's carbonyl or amine frame.
    Internal rows and ordinary peptide junctions retain their existing parameters.
    """
    if profile.name != "nonstandard_aa":
        return params
    extra = {"length_parameters": [], "angle_parameters": []}
    for connection, role, partner_role in (("down", "N", "C"), ("up", "C", "N")):
        endpoint = getattr(profile, connection)
        if endpoint is None:
            continue
        atom = renamed.get(endpoint[1], endpoint[1])
        if references.get(atom, atom) == role:
            continue
        stub = getattr(profile, connection + "_partner")
        cross = {stub: "+" + partner_role}
        for cap in profile.caps:
            if cap.bond_to == stub and cap.element in ("C", "O"):
                cross[cap.name] = "+CA" if cap.element == "C" else "+O"
        for bond in capped_params.length_parameters:
            neighbor = (
                bond.atm2
                if bond.atm1 == stub
            ...
            if elements.get(neighbor) == "H":
                cross[neighbor] = "+H"
        for field, n_atoms in (("length_parameters", 2), ("angle_parameters", 3)):
            for param in getattr(capped_params, field):
                names = [getattr(param, f"atm{i}") for i in range(1, n_atoms + 1)]
                mapped = [cross.get(name, renamed.get(name, name)) for name in names]
                if (
                    any(name in cross for name in names)
                    and any(name in kept for name in mapped)
                    and all(name in kept or name in cross.values() for name in mapped)
                ):
                    extra[field].append(
                        attr.evolve(
                            param,
                            **{f"atm{i}": name for i, name in enumerate(mapped, 1)},
                        )
                    )
    updates = {
        field: getattr(params, field) + tuple(rows)
        for field, rows in extra.items()
        if rows
    }
    return attr.evolve(params, **updates) if updates else params
