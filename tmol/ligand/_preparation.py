...
def _assert_fragment_names_available(param_db, fragment_preparations) -> None:
    ...
    for prep in fragment_preparations:
        ...
        if previous is None:
            continue
        previous_charges = charges_for_database_residue(
...
def _canonical_conjugation_parameters(
    ...
    for res_name, partners in sorted(sites.items()):
        # Every base type sharing this io-equivalence class needs its own patch.
        # Taking only the first gives histidine HIS, whose single ring proton is
        # HE2 on NE2, so a ligand bonded at NE2 displaces it and leaves neither
        # ring nitrogen protonated -- an imidazolate -- while the charge
        # bookkeeping still conserves the net charge of the molecule that had
        # the proton. HIS_D carries HD1 on ND1 instead, and only it is right for
        # that site. conjugation_patch scopes each patch to its own base name,
        # so emitting all of them is self-deduplicating.
        bonded_records = param_db.scoring.cartbonded.residue_params
        residue_types = [
            r
            for r in param_db.chemical.residues
            if r.io_equiv_class == res_name and r.name == r.base_name
            # A base type with no bonded baseline cannot be parameterized once
            # patched. HIS_POS has none, so only HIS and HIS_D are candidates --
            # which is the pair the ND1/NE2 distinction needs anyway.
            and r.name in bonded_records
        ]
        if not residue_types:
            logger.warning(
                "No base residue type for conjugation site %s; its attachments "
                "are not parameterized",
                res_name,
            )
            continue
        for residue_type in residue_types:
            ...
            for atom in sorted(partners):
                ...
                base_hydrogens = len(
                    _hydrogens_on(residue_type, atom, param_db.chemical)
                )
                is_terminal_amine = (
                ...
                try:
                    ...
                    # A base type of this class whose site cannot be framed
                    # cannot take the attachment at all: CYD is the
                    # disulfide-bonded form of cysteine, so a ligand at SG is on
                    # CYS. Its siblings are still patched.
                    logger.info(
                        "No attachment patch for %s.%s: %s",
                        residue_type.name,
                        atom,
                        error,
                    )
...
def prepare_ligands(  # noqa: C901
    ...
    for ligand_name, layout in fragment_layouts_by_name.items():
        ...
        fragment_source_preparations[ligand_name] = LigandPreparation(
            residue_type=restype,
            partial_charges=charges_for_database_residue(param_db, ligand_name),
            cartbonded_params=cartbonded_params,
        ...
        if not preparations:
            ...
            preparations.append(
                LigandPreparation(
                    ...
                    partial_charges=charges_for_database_residue(
