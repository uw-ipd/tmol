...
def _unstructure_residue(rt: RawResidueType) -> dict[str, Any]:
    ...
    # A connection's bond is not in a ring unless a cut through one made it so.
    # Emitting the default on every connection of every file buys nothing; the
    # reader supplies it.
    for conn in d.get("connections", []):
        if isinstance(conn, dict) and not conn.get("in_ring"):
            conn.pop("in_ring", None)
