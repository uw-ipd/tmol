...
def _connection_record(mol, props, first, second, maps, names, adjacency, ideals=None):
    ...
    try:
        ...
        for side in (0, 1):
            ...
            polymer_ports = [
                connection
                for connection in (first, second)[side].connections
                if connection.atom == roots[side]
                and connection.name in ("up", "down")
                and connection.name != names[side]
            ]
            # An internal N attachment's capped chemical model includes the
            # incoming carbonyl. That atom belongs to a third block in the pose,
            # so it cannot become an angle in this two-block connection record.
            # Keep it for protonation/typing, but accept only unmapped synthetic
            # cap neighbors accounted for by another polymer port at this atom.
            allowed_cap_neighbors = len(extra) <= len(
                polymer_ports
            ) and extra.isdisjoint(maps[side].values())
            if expected - actual or (extra and not allowed_cap_neighbors):
                raise ValueError(
...
def _model_members(model, candidates_by_site):
    from tmol.ligand._conjugate_context import model_attachment_contexts
    ...
    contexts = model_attachment_contexts(model)
    for ri, types in candidates.items():
        if any(rt.conjugation_context for rt in types):
            candidates[ri] = [
                rt for rt in types if rt.conjugation_context == contexts[ri]
            ]
...
def _model_records(
    ...
    *,
    ...
    for ri, types in candidates.items():
        ...
        identity = (
            types[0].base_name,
            tuple(sorted(sites[ri])),
            types[0].conjugation_context,
        )
        first = locals_by_residue[ri][0]
        instance = (
            f"{model.atom_array.chain_id[first]}:"
            f"{model.atom_array.res_id[first]}{model.atom_array.ins_code[first]}"
        )
        if identity in contexts:
            previous, previous_instance = contexts[identity]
            if previous != signature:
                old, new = (
                    {row[0]: row[1:] for row in previous},
                    {row[0]: row[1:] for row in signature},
                )
                changed = sorted(
                    name
                ...
                raise ValueError(
                    ...
                    f"{previous_instance} and {instance} differ at {', '.join(changed)}. "
                    "Correct the source stereochemistry/chemistry or supply distinct residue identities."
                )
        else:
            contexts[identity] = signature, instance
        for rt in types:
...
def generate_conjugate_connection_params(
    ...
    *,
    ...
    seen = set()
    for model in (
        iter_capped_conjugate_models(atom_array, chem) if _models is None else _models
    ):
