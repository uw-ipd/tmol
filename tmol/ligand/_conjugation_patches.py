...
def _open_valence_frame(residue_type, atom, chemdb, distance, geometry=None):
    ...
    A planar three-neighbor center has no unique side to attach to. A one-neighbor
    ...
    if len(neighbors) == 1 and geometry is not None:
        parent, target_angle, target_length = geometry
        # A deprotonated oxygen has no departing H to supply its frame. Use
        # the conjugate generator's angle; the linkage chi owns its free torsion.
        gp = neighbors[0]
        ...
        plane_refs = sorted(
            ...
            for first, second, *_ in residue_type.bonds
            for this, other in ((first, second), (second, first))
            if this == gp and other != atom and element[other] != "H"
        )
        ggp = next(
            (
                n
                for n in plane_refs
                if np.linalg.norm(np.cross(xyz[atom] - xyz[gp], xyz[n] - xyz[gp]))
                > 1e-6
            ),
            None,
        )
        if gp == parent and ggp is not None:
            return Icoor(
                ...
                phi=0.0,
                theta=np.pi - target_angle,
                d=float(distance if distance is not None else target_length),
            )
...
def conjugation_patch(
    ...
    frame = (
        ...
        else _open_valence_frame(residue_type, atom, chemdb, distance, open_geometry)
    )
    if frame is None:
        return None
    ...
    if leaving is not None:
        # The connection lies on the departing atom's ray. With that ray's
        # origin as grandparent, replacing the dihedral reference preserves geometry.
        icoors += tuple(
            IcoorVariant(
                name=f"<{ic.name}>",
                source=f"<{ic.name}>",
                great_grand_parent=name,
            )
            for ic in residue_type.icoors
            if ic.name not in gone
            and ic.grand_parent == atom
            and ic.great_grand_parent == leaving
            and ic.parent not in gone
        )
...
def combined_terminal_conjugation_patch(
    residue_type, terminal_type, conjugation, chemdb
):
    """Compose a terminal form and N attachment as one base-relative patch.

    The ordinary terminus and conjugation patches both modify the attachment
    nitrogen, so the patch engine intentionally refuses to apply them in
    sequence. Build the chemically desired target with a fresh modification
    history, then emit only its difference from the unpatched residue. This
    leaves unrelated attachment icoors untouched and able to compose normally.
    """
    from tmol.database._patched_chemdb import RestypeGraphBuilder, do_patch

    elements = {atom_type.name: atom_type.element for atom_type in chemdb.atom_types}
    builder = RestypeGraphBuilder(elements)
    # The terminal molecule already supplied the correct physical amine type.
    # Conjugation removes one proton but must not turn that free amine into Nbb.
    terminal_conjugation = attr.evolve(conjugation, modify_atoms=())
    targets, _marked = do_patch(
        terminal_type,
        terminal_conjugation,
        builder.from_raw_res(terminal_type),
        None,
        [],
    )
    if len(targets) != 1:
        return None
    target = targets[0]
    old_atoms = {atom.name: atom for atom in residue_type.atoms}
    new_atoms = {atom.name: atom for atom in target.atoms}
    old_connections = {conn.name: conn for conn in residue_type.connections}
    new_connections = {conn.name: conn for conn in target.connections}
    retained = old_atoms.keys() & new_atoms.keys()
    old_bonds = {bond for bond in residue_type.bonds if set(bond[:2]) <= retained}
    new_bonds = {bond for bond in target.bonds if set(bond[:2]) <= retained}
    if old_bonds != new_bonds:
        raise ValueError(
            f"Terminal conjugation changes retained bonds in {residue_type.name}"
        )

    added = new_atoms.keys() - old_atoms.keys()

    def reference(name):
        return f"<{name}>" if name in old_atoms or name in old_connections else name

    old_icoors = {icoor.name: icoor for icoor in residue_type.icoors}
    icoors = tuple(
        IcoorVariant(
            name=reference(icoor.name),
            phi=icoor.phi,
            theta=icoor.theta,
            d=icoor.d,
            parent=reference(icoor.parent),
            grand_parent=reference(icoor.grand_parent),
            great_grand_parent=reference(icoor.great_grand_parent),
        )
        for icoor in target.icoors
        if icoor != old_icoors.get(icoor.name)
    )
    return VariantType(
        name=f"{CONNECTION_PREFIX}terminal_{residue_type.base_name}_{conjugation.display_name}",
        display_name=f"{conjugation.display_name}:nterm",
        pattern="",
        remove_atoms=tuple(
            reference(name)
            for name in (
                *sorted(old_atoms.keys() - new_atoms.keys()),
                *sorted(old_connections.keys() - new_connections.keys()),
            )
        ),
        add_atoms=tuple(atom for atom in target.atoms if atom.name in added),
        add_atom_aliases=tuple(
            alias
            for alias in target.atom_aliases
            if alias.name in added and alias not in residue_type.atom_aliases
        ),
        modify_atoms=tuple(
            attr.evolve(atom, name=reference(atom.name))
            for atom in target.atoms
            if atom.name in retained and atom != old_atoms[atom.name]
        ),
        add_connections=tuple(
            attr.evolve(conn, atom=reference(conn.atom))
            for conn in target.connections
            if conn.name not in old_connections
        ),
        add_bonds=tuple(
            (reference(first), reference(second), *rest)
            for first, second, *rest in target.bonds
            if first in added or second in added
        ),
        icoors=icoors,
        add_torsions=tuple(
            torsion
            for torsion in target.torsions
            if torsion not in residue_type.torsions
        ),
        add_chi_samples=tuple(
            sample
            for sample in target.chi_samples
            if sample not in residue_type.chi_samples
        ),
        applies_to=VariantScope(base_names=(residue_type.base_name,)),
    )
