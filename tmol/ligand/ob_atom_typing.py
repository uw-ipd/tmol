...
# Static polar-neighbor set (matches chemistry_tables._LEGACY_POLAR_CLASSES).
...
# Rosetta Types.SPECIAL_HYBRIDS (mol2 Tripos column)
_MOL2_SPECIAL_HYBRIDS: dict[str, int] = {
    "N.pl3": 2,
    "N.am": 8,
    "N.aro": 2,
    "C.cat": 2,
...
def hyb_from_mol2_sybyl(sybyl: str) -> int:
    """Map a mol2 Tripos type string to Rosetta ``hyb`` (``Molecule.read_mol2``)."""
    s = (sybyl or "").strip()
    if not s:
        return 0
    if s in _MOL2_SPECIAL_HYBRIDS:
        return _MOL2_SPECIAL_HYBRIDS[s]
    key = s.lower()
    if key in _MOL2_SPECIAL_HYBRIDS:
        return _MOL2_SPECIAL_HYBRIDS[key]
    if "." in s:
...
_mol2_hyb_by_idx: dict[int, int] | None = None


def _get_hyb(obatom: openbabel.OBAtom) -> int:
    """Map hybridization to Rosetta mol2 convention.

    When ``_mol2_hyb_by_idx`` is set (mol2 Tripos column), that wins over
    OpenBabel perception — matching Rosetta ``Molecule.read_mol2``.
    """
    idx = obatom.GetIndex()
    if _mol2_hyb_by_idx is not None and idx in _mol2_hyb_by_idx:
        hyb = _mol2_hyb_by_idx[idx]
        if hyb != 0:
            return hyb
...
def assign_obmol_atom_types(
    obmol: openbabel.OBMol,
    *,
    mol2_hyb_by_idx: dict[int, int] | None = None,
) -> list[AtomTypeAssignment]:
    ...
    prev_hyb = _mol2_hyb_by_idx
    _mol2_hyb_by_idx = mol2_hyb_by_idx
