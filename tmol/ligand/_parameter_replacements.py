...
def _local_identity(restype, charges, bonded):
    ...
    definition = cattr.unstructure(restype)
    if not definition["conjugation_context"]:
        definition.pop("conjugation_context")
    # An absent optional reference must not invalidate existing v1 bundles.
    for atom in definition["atoms"]:
        if atom["cartbonded_reference"] is None:
            atom.pop("cartbonded_reference")
