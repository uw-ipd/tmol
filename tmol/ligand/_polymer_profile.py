...
# Deuterium is hydrogen. Treating it as a heavy atom in one place and not
# another makes the representative-copy choice and the capping disagree about
# the same atom of a neutron structure.
...
def _heavy_adjacency(atom_array):
    ...
    for i, j, order in atom_array.bonds.as_array():
        ...
        if element.get(a) in _HYDROGEN_ELEMENTS or element.get(b) in _HYDROGEN_ELEMENTS:
            continue
...
def _nitrogen_has_hydrogen_capacity(atom_array, nitrogen: str) -> Optional[bool]:
    """Whether an amine-like nitrogen carries or can carry a hydrogen.

    True and False are supported candidate and supported saturated states.
    None means the topology or charge is unsupported and must not imply a cap.
    """
    names = {i: str(n) for i, n in enumerate(atom_array.atom_name)}
    indices = {name: i for i, name in names.items()}
    elements = {
        str(n): str(e) for n, e in zip(atom_array.atom_name, atom_array.element)
    }
    index = indices.get(nitrogen)
    if index is None or atom_array.bonds is None:
        return None
    ...
    for i, j, order in atom_array.bonds.as_array():
        ...
        if index not in (i, j):
            continue
        other = names[j if i == index else i]
        if elements.get(other) in _HYDROGEN_ELEMENTS:
            return True
        heavy_bonds.append((other, int(order)))

    # Count what each bond contributes to the nitrogen's valence. A nitrogen
    # multiply bonded inside a ring is described chemistry, not unsupported
    # chemistry: the ring already spoke for its valence, so it is saturated
    # rather than unknown. The same bond to a substituent outside every ring is
    # different -- on a heavy-only residue nothing says whether it is real or a
    # mis-typed single bond -- so that one leaves the answer open.
    contributions = {
        int(struc.BondType.SINGLE): 1.0,
        int(struc.BondType.DOUBLE): 2.0,
        int(struc.BondType.TRIPLE): 3.0,
        int(struc.BondType.AROMATIC): 1.5,
        int(struc.BondType.AROMATIC_SINGLE): 1.5,
        int(struc.BondType.AROMATIC_DOUBLE): 1.5,
    }
    if not heavy_bonds or any(order not in contributions for _, order in heavy_bonds):
        return None
    # An aromatic nitrogen with no hydrogen is saturated whether the ring leaves
    # it two sigma bonds, as pyridine does, or three, as a purine N9 does. Adding
    # its bond orders instead puts N9 over the target valence and reports
    # unsupported chemistry, which loses a terminal G or A its 5' end; counting
    # only sigma bonds would go the other way and offer pyridine a hydrogen.
    aromatic = (
        int(struc.BondType.AROMATIC),
        int(struc.BondType.AROMATIC_SINGLE),
        int(struc.BondType.AROMATIC_DOUBLE),
    )
    if any(order in aromatic for _, order in heavy_bonds):
        return False
    # A multiple bond to a carbon that has no other heavy neighbour is the one
    # shape that cannot be real: a methyl carbon has nothing to double-bond
    # with. Ring membership is the wrong test here -- an imine like LLP's Schiff
    # base and a nitroso like SNC's both sit outside any ring and are perfectly
    # ordinary chemistry, and rejecting them reports their nitrogens as
    # unsupported. Over the whole dictionary this test changes no answer; it
    # exists to refuse a bond order nothing could have meant.
    heavy_degree: dict = {}
    for first, second, _order in atom_array.bonds.as_array():
        for owner, neighbour in ((int(first), int(second)), (int(second), int(first))):
            if elements.get(names[neighbour]) not in _HYDROGEN_ELEMENTS:
                heavy_degree[names[owner]] = heavy_degree.get(names[owner], 0) + 1
    if any(
        order != int(struc.BondType.SINGLE)
        and elements.get(other) == "C"
        and heavy_degree.get(other, 0) <= 1
        for other, order in heavy_bonds
    ):
        return None
    if "charge" not in atom_array.get_annotation_categories():
        return None
    charge = atom_array.charge[index]
    target_valence = {0: 3, 1: 4}.get(charge)
    if target_valence is None:
        return None
    valence = sum(contributions[order] for _, order in heavy_bonds)
...
def _chain_end_candidates(atom_array, known: str) -> Optional[list]:
    """Atoms that could carry the backbone's other polymer connection.

    An empty list means the residue does not continue the chain at all: it is
    a cap, terminating whatever it is bonded to. Exactly one candidate is a
    chain member seen at a terminus. More than one cannot be told apart. None
    means at least one nitrogen has unsupported chemistry, which must not imply
    a cap.
    """
    adj, double, element = _heavy_adjacency(atom_array)
    if element.get(known) == "N":
        ...
        states = {
            n: _nitrogen_has_hydrogen_capacity(atom_array, n)
            for n in sorted(element)
            if element.get(n) == "N" and n != known
        }
        if any(state is None for state in states.values()):
            return None
        candidates = [n for n, state in states.items() if state]
...
def canonical_backbone_renames(atom_array, profile, chemdb, connection_atoms=None):
    ...
    if profile.polymer_type == "nucleic_acid":
        adjacency, double, elements = _heavy_adjacency(atom_array)
        path, _ = na_sugar_mainchain(adjacency, elements)
        if (
            path is not None
            and tuple(path[1:]) == profile.mainchain_atoms[1:]
            and elements.get(path[0]) == "P"
        ):
            phosphate = profile.mainchain_atoms[0]
            if path[0] != phosphate:
                if phosphate in names or phosphate in renames.values():
                    raise ValueError(
                        f"{atom_array.res_name[0]}: multiple atoms map to phosphate {phosphate}"
                    )
                renames[path[0]] = phosphate
            source_oxygens = sorted(
                (
                    n
                    for n in adjacency[path[0]]
                    if elements.get(n) == "O"
                    and len(adjacency[n]) == 1
                    and n not in renames
                    and n not in backbone
                ),
                key=lambda n: (n not in double.get(path[0], ()), n),
            )
            available = (
                backbone - set(profile.mainchain_atoms) - names - set(renames.values())
            )
            target_oxygens = sorted(
                b if a == phosphate else a
                for a, b, *_ in reference.bonds
                if phosphate in (a, b) and (b if a == phosphate else a) in available
            )
            if len(source_oxygens) == len(target_oxygens):
                renames.update(zip(source_oxygens, target_oxygens))
    return renames
...
# Wildcard row groups: attribute, atom count, and whether the
...
def substituted_wildcard_rows(cartbonded_db, canonical, replacement, present):
    ...
    ``canonical`` is one name or several standing in for a single atom.
    A row naming two of them describes a different residue and is omitted.
...
def noncanonical_junction_substitutions(residue_type, atom_type_index, source=None):
    ...
    """
    # Use retained atoms, including generated H, but prefer source bond orders:
    # Rosetta labels some conjugated single bonds DOUBLE for parameter output.
    ...
    conns = {c.name: c.atom for c in residue_type.connections}
    source_orders = (
        {
            frozenset((source.atom_name[a], source.atom_name[b])): struc.BondType(
                order
            ).name
            for a, b, order in source.bonds.as_array()
        }
    ...
    for a, b, order, *_ in residue_type.bonds:
        order = source_orders.get(frozenset((a, b)), order)
        neighbors[a][b] = order
        neighbors[b][a] = order
    ...
    subs = []
    upper = conns.get("up")
    if upper == mainchain[-1] and elements.get(upper) == "C":
        ...
        carbons = sorted(
            atom
            for atom, order in bonded.items()
            if elements[atom] == "C" and order == "SINGLE"
        )
        before = mainchain[-2] if len(mainchain) > 1 else next(iter(carbons), None)
        if len(oxygens) == 1 and (before is None or before in carbons):
            mapping = {"C": upper, "O": oxygens[0]}
            if before is not None:
                mapping["CA"] = before
            retain(mapping)

    lower = conns.get("down")
    if lower == mainchain[0] and elements.get(lower) == "N":
        bonded = neighbors[lower]
        carbons = sorted(atom for atom in bonded if elements[atom] == "C")
        after = mainchain[1] if len(mainchain) > 1 else next(iter(carbons), None)
        if (after is None or after in carbons) and all(
            order == "SINGLE" for order in bonded.values()
        ):
            mapping = {"N": lower}
            if after is not None:
                mapping["CA"] = after
            # A cyclic amine cap has the same second N neighbor as proline.
            # Its own source name must not suppress that angle or improper.
            if len(mainchain) == 1 and len(carbons) == 2:
                other = next(atom for atom in carbons if atom != after)
                ring = any(
                    {a, b} == {lower, other} and in_ring
                    for a, b, _order, in_ring in residue_type.bonds
                )
                mapping["CD" if ring else "CN"] = other
            hydrogens = sorted(atom for atom in bonded if elements[atom] == "H")
            if hydrogens:
                mapping["H"] = "H" if "H" in hydrogens else hydrogens[0]
            retain(mapping)
            if len(mainchain) == 1:
                # A primary amide cap has two equivalent N-bound hydrogens.
                subs.extend({"H": h} for h in hydrogens if h != mapping.get("H"))
    return subs
...
def cap_residue(atom_array, profile: PolymerProfile, *, include_coordinates=True):
    ...
    keep = numpy.array([str(e) not in _HYDROGEN_ELEMENTS for e in atom_array.element])
