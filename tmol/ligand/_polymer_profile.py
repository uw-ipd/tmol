...
def _nitrogen_has_hydrogen_capacity(atom_array, nitrogen: str) -> Optional[bool]:
    ...
    for first, second, _order in atom_array.bonds.as_array():
        for index, other in ((int(first), int(second)), (int(second), int(first))):
            if elements.get(names[other]) not in _HYDROGEN_ELEMENTS:
                heavy_degree[names[index]] = heavy_degree.get(names[index], 0) + 1
    if any(
