...
def _nitrogen_has_hydrogen_capacity(atom_array, nitrogen: str) -> Optional[bool]:
    ...
    for i, j, order in atom_array.bonds.as_array():
        ...
        if elements.get(other) in ("H", "D"):
            return True
        heavy_bond_orders.append(int(order))

    if not heavy_bond_orders or any(
        order != int(struc.BondType.SINGLE) for order in heavy_bond_orders
    ):
        return None
    ...
    if target_valence is None:
        return None
    if len(heavy_bond_orders) < target_valence:
        return True
    if len(heavy_bond_orders) == target_valence:
        return False
    return None
