...
struct AllPairsShortestPathsDispatch {
  ...
  )
  {
	...
	for (int kk = 0; kk < TILE_SIZE; ++kk) {
	  ...
	  __sync_threads(); // wait for read

	  if (xx_kk_weight >= 0 && kky_weight >= 0) {
	    if (x_y_curr < 0 || xkk_weight + kky_weight < x_y_curr) {
	      x_y_curr = xkk_weight + kky_weight;
	      shared_weights[TILE_SIZE * x + y] = x_y_curr;
	    }
	  }
	  __sync_threads(); // wait for write
	}
	...
	// If Z is 0, then we are targetting a block in the same column as the primary
	// block. If Z is 1, then we are targetting a block in the same row.
	int const i = TILE_SIZE * (z == 0 ? off_diag_block + is_after_primary : primary_block) + x;
	int const j = TILE_SIZE * (z == 1 ? off_diag_block + is_after_primary : primary_block) + y;
	...
	if (primary_i < max_n_nodes && primary_j < max_n_nodes) {
	  shared.primary_block_weights[TILE_SIZE * x + y] = weights[graph_ind][primary_i][primary_j];
	} else {
	...
	// Now iterate across all k in the primary row (column if z = 1) and see if the
	if (z == 0) {
	...
	int const graph_ind = blockIdx.x;
	int const target_block_row = blockIdx.y + (blockIdx.y >= primary_block ? 1 : 0);
	int const target_block_column = blockIdx.z + (blockIdx.z >= primary_block ? 1 : 0);
	int const x = threadIdx.x;
	int const y = threadIdx.y;

	int const primary_i = TILE_SIZE * primary_block + x;
	int const primary_j = TILE_SIZE * primary_block + y;
	int const i = TILE_SIZE * target_row + x;
	int const j = TILE_SIZE * target_column + y;
	...
	if (primary_i < max_n_nodes && j < max_n_nodes) {
	  shared.ph2_column_block[TILE_SIZE * x + y] = weights[graph_ind][primary_i][j];
	} else {
	...
	if (i < max_n_nodes && primary_j < max_n_nodes) {
	  shared.ph2_row_block[TILE_SIZE * x + y] = weights[graph_ind][i][primary_j];
	} else {
	...
	Int const x_y_curr = weights[graph_ind][i][j];

	// Now 
	for (int k = 0; k < TILE_SIZE; ++k) {
	  Int const ik_weight = shared.p2_column_block[TILE_SIZE * x + k];
	  Int const jk_weight = shared.p2_row_block[TILE_SIZE * k + y];
	  if (ik_weight >= 0 && jk_weight >= 0) {
    ...
    // OK! Now we need to iterate across the blocks and launch the three phases per block

    auto p1_kernel = (__global__ [=](int primary_block) {phase1(primary_block);});
    auto p2_kernel = (__global__ [=](int primary_block) {phase2(primary_block);});
    auto p3_kernel = (__global__ [=](int primary_block) {phase3(primary_block);});
    
    int const n_blocks = (max_n_nodes-1) / TILE_SIZE + 1;
    int const n_threads = TILE_SIZE * TILE_SIZE;

    for (int i = 0; i < n_blocks; ++i) {
      p1_kernel<<<1, n_threads>>>(i);
      if (n_blocks > 1) {
	dim3 p2_dim(n_graphs, n_blocks - 1, 2);
	p2_kernel<<<p2_dim, n_threads>>>(i);

	dim3 p3_dim(n_graphs, n_blocks - 1, n_blocks - 1);
	p3_kernel<<<p3_dim, n_threads>>>(i);
      }
    }
