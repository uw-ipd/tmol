...
struct PickRotamers {
  ...
  ) -> void
  {
	...
	alternate_coords[alt_offset + atom_id][0] = context_coords[i_context][context_offset + atom_id][0];
	alternate_coords[alt_offset + atom_id][1] = context_coords[i_context][context_offset + atom_id][1];
	alternate_coords[alt_offset + atom_id][2] = context_coords[i_context][context_offset + atom_id][2];
      } else {
	int const alt_offset = alternate_coord_offsets[alt_id];
	Int i_rot = random_rots[i_context];
	int const rotamer_offset = rotamer_coord_offsets[i_rot];
	alternate_coords[alt_offset + atom_id][0] = rotamer_coords[rotamer_offset + atom_id][0];
	alternate_coords[alt_offset + atom_id][1] = rotamer_coords[rotamer_offset + atom_id][1];
	alternate_coords[alt_offset + atom_id][2] = rotamer_coords[rotamer_offset + atom_id][2];
      }
    };

    Dispatch<D>::forall(n_contexts* 2 * max_n_atoms_per_block, copy_rotamer_coords);
