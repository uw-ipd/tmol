// compiled.mps.mm — MPS instantiation of pack/compiled dispatch structs.
//
// AnnealerDispatch and InteractionGraphBuilder contain pure C++ logic with
// no CUDA intrinsics.  We include the shared impl header and instantiate
// for Device::MPS.
...
// AnnealerDispatch for MPS reuses the same CPU SA implementation.
// The SA algorithm is sequential by nature and runs on the host CPU; tensor
// data is accessible via unified memory on Apple Silicon.
...
auto AnnealerDispatch<D>::forward(
    ...
    TView<int, 1, D> rotamer_offset_for_pose,
  ...
  clock_t stop = clock();
  std::cout << "MPS simulated annealing (CPU path) in "
...
template struct AnnealerDispatch<tmol::Device::MPS>;
