...
struct AnnealerDispatch
{
      ...
      TPack<int, 2, D> >
  {
    ...
    auto ibr_rotamer_assignments_t = TPack<int, 2, D>::zeros({n_ibr_traj, nres});
    ...
    auto ibr_rotamer_assignments = ibr_rotamer_assignments_t.view;
    ...
    auto faster_iBR = [=] MGPU_DEVICE (int thread_id) {
      ...
      int prev_run_sorted_id = warp_id / n_simA_expansions_for_ibr;
      int prev_run_id = sorted_assignment_inds[prev_run_sorted_id];
      for (int i = g.thread_rank(); i < nres; i += 32) {
        int i_rot = rotamer_assignments[prev_run_id][i];
        ibr_rotamer_assignments[warp_id][i] = i_rot;
        best_ibr_rotamer_assignments[warp_id][i] = i_rot;
      }
      float energy = total_energy_for_assignment_parallel(g,
        nrotamers_for_res, oneb_offsets, res_for_rot, nenergies, twob_offsets,
        energy1b, energy2b, ibr_rotamer_assignments[warp_id]);
      ...
      for (int ibr_iteration = 0; ibr_iteration < n_ibr; ++ibr_iteration) {
        ...
        for (int i = 0; i < nres; ++i) {
          ...
          int i_nrots = nrotamers_for_res[i];
          int i_curr_rot = ibr_rotamer_assignments[warp_id][i];
          ...
          for (int j = g.thread_rank(); j < i_nrots; j += 32) {
            ...
            for (int k = 0; k < nres; ++k) {
              ...
              int const k_rotamer = ibr_rotamer_assignments[warp_id][k];
              jE += energy2b[
                twob_offsets[k][i] +
                i_nrots * k_rotamer + j];
            }
          ...
          if (mine_is_best && scan_val == 1) {
            // exactly one thread saves the assigned rotamer to the
            // ibr_perturbed_assignemnt array
            ibr_perturbed_assignments[warp_id][i] = my_best_rot;
        ...
        float alt_energy = total_energy_for_assignment_parallel(g,
          nrotamers_for_res, oneb_offsets, res_for_rot, nenergies, twob_offsets,
          energy1b, energy2b, ibr_perturbed_assignments[warp_id]);
        ...
        for (int i = g.thread_rank(); i < nres; i += 32) {
          int i_rot = ibr_perturbed_assignments[warp_id][i];
          ibr_rotamer_assignments[warp_id][i] = i_rot;
          if (accept_as_best) {
            best_ibr_rotamer_assignments[warp_id][i] = i_rot;
          }
        ...
        int const max_hist_to_check = min(ibr_history_size, ibr_iteration-1);
        bool exit_ibr_loop = true;
        for (int i = 0; i < max_hist_to_check; ++i) {
          int const i_hist = (ibr_iteration - i - 1) % ibr_history_size;
          exit_ibr_loop = true;
          for (int j = g.thread_rank(); j < nres; j += 32) {
            if (ibr_rotamer_assignments[warp_id][j] !=
              ibr_assignment_history[warp_id][i_hist][j]) {
              exit_ibr_loop = false;
            }
        ...
        int const ibr_it_hist = ibr_iteration % ibr_history_size;
        for (int i = g.thread_rank(); i < nres; i += 32) {
          ibr_assignment_history[warp_id][ibr_it_hist][i] =
            ibr_rotamer_assignments[warp_id][i];
        }
