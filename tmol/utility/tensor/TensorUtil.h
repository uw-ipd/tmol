...
tmol::PackedTensorAccessor<ToT, N> reinterpret_tensor(at::Tensor input_t) {
  ...
  int64_t sizes[N];
  int64_t strides[N];
  static_assert(sizeof(ToT) % sizeof(FromT) == 0, "Cast target type must be even multiple size of source type.");

  int64_t stride_factor = sizeof(ToT) / sizeof(FromT);

  AT_ASSERT(input.size(N-1) % stride_factor == 0, "Low-dimension shape must be even multiple of adjusted stride.")
  ...
  return tmol::PackedTensorAccessor<ToT, N>( reinterpret_cast<ToT*>(input_t.data_ptr()), sizes, strides);
}
