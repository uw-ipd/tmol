...
def find_disulfides(
    ...
    if disulfides is not None:
        endpoints = torch.cat((disulfides[:, :2], disulfides[:, (0, 2)]))
        endpoints, counts = torch.unique(endpoints, dim=0, return_counts=True)
        if torch.any(counts > 1):
            raise ValueError(
                "Each cysteine may occur in only one declared disulfide; "
                f"repeated (pose, residue) endpoints: {endpoints[counts > 1].tolist()}"
            )
