"""Complete anchored ligand atoms from their prepared conformer geometry."""
...
def build_missing_nonpolymer_atoms(pbt, coords, targets, offsets, types, connections):
    ...
    other states. An isolated single-heavy-atom molecule with no supplied
    hydrogens starts in its ideal conformer's orientation. Other underdetermined
    or degenerate frames stay NaN.
    ...
    for pi, bi in torch.nonzero(targets.any(-1)).tolist():
        ...
        if bt.properties.polymer.is_polymer and bt.properties.is_canonical:
            continue
        ...
        if not torch.isfinite(ideal).all():
            continue
        hydrogen = pbt.atom_is_hydrogen[ti, : bt.n_atoms].bool()
        missing = torch.nonzero(
            targets[pi, bi, : bt.n_atoms] & torch.isnan(current).any(-1)
        ).flatten()
        if (
            len(names) == 1
            and int((~hydrogen).sum()) == 1
            and not torch.any(finite & hydrogen)
            and not torch.any(connections[pi, bi, :, 0] >= 0)
        ):
            generated = (
                ideal[bt.at_to_icoor_ind]
                - ideal[bt.icoors_index[names[0]]]
                + observed[0]
            )
            updates.append(generated[missing].to(dtype=coords.dtype))
            indices.append(
                torch.stack((torch.full_like(missing, pi), offset + missing))
            )
            continue
        if len(names) < 2:
            continue
