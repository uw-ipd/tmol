...
class RosettaHBParams:
    ...
    def _load_tables(self):
        return RawParams(
            **{
                t: pandas.read_csv(f"{self.path}/{t}.csv", header=None, names=table_schema[t])
    ...
    def _load_donor_types(self):
        donor_table = (pandas.merge(self.tables.HBDonChemType,
                                    pandas.DataFrame({
    ...
    def _load_acceptor_types(self):
        acceptor_table = (toolz.reduce(
            pandas.merge, (self.tables.HBAccChemType.drop(["id"], axis="columns"),
                           pandas.DataFrame({
                               "name": self.target_acceptors
    ...
    def _load_pair_params(self):
        ...
        pair_param_table = toolz.reduce(
            pandas.merge, (self.tables.HBEval.drop(
                filter(lambda c: c.endswith("fade"), self.tables.HBEval.columns), axis="columns")
                           .drop(["weight_type", "separation", "comment"], axis="columns")
                           .drop_duplicates(), self.donor_types["name"].to_frame("don_chem_type"),
                           self.acceptor_types["name"].to_frame("acc_chem_type")))

        assert (len(pair_param_table.groupby(
            ["don_chem_type",
             "acc_chem_type"])) == len(self.target_acceptors) * len(self.target_donors))

        return pair_param_table

    polynomial_parameters: pandas.DataFrame = attr.ib()

    @polynomial_parameters.default
    def _load_polynomial_params(self):
        term_names = ("AHdist", "cosBAH_short", "cosBAH_long", "cosBAH2_long", "cosAHD_short",
                      "cosAHD_long")

        poly_names = toolz.reduce(set.union, (set(self.pair_params[c].values) for c in term_names))
    ...
    def to_yaml(self, outfile = None):
        ...
        with p.group(2, ):
            ...
            _("sp2_acceptors:")
            with p.group(2):
                for i, a in self.acceptor_types.query("hybridization == 'SP2_HYBRID'").iterrows():
                    _(f"- {a['name']} #{a['comment']}")

            _("sp3_acceptors:")
            with p.group(2):
                for i, a in self.acceptor_types.query("hybridization == 'SP3_HYBRID'").iterrows():
                    _(f"- {a['name']} #{a['comment']}")

            _("ring_acceptors:")
            with p.group(2):
                for i, a in self.acceptor_types.query("hybridization == 'RING_HYBRID'").iterrows():
                    _(f"- {a['name']} #{a['comment']}")
