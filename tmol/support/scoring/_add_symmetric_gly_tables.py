...
def check_registration(table) -> None:
    """Refuse a table whose bins are not the ones ``mirror`` assumes.

    ``mirror`` reflects by index, which is the reflection through the origin
    only for a grid that starts at -pi and spans exactly one period with no
    duplicated wrap row. Checking the symmetrized table against its own mirror
    cannot establish that: ``symmetrize_energies`` averages a table with its
    mirror, and ``mirror`` is an involution, so the result is symmetric under
    it by construction and the comparison is true for every input, including a
    table registered somewhere else entirely. The bbdep-omega tables start at
    pi/36, so a table of that provenance is a real thing to guard against.
    """
    bins = numpy.asarray(table.table).shape[:2]
    for axis, (start, step, n) in enumerate(zip(table.bbstart, table.bbstep, bins)):
        if not numpy.isclose(start, -numpy.pi):
            raise ValueError(
                f"{table.table_id} axis {axis} starts at {start:g}, not -pi; "
                "reflecting it by index would mirror through the wrong point"
            )
        if not numpy.isclose(n * step, 2 * numpy.pi):
            raise ValueError(
                f"{table.table_id} axis {axis} spans {n * step:g} over {n} bins, "
                "not one period; it may carry a duplicated wrap row"
            )
...
def symmetrize_energies(energies: numpy.ndarray) -> numpy.ndarray:
    ...
    The tables hold -ln(p) plus a constant, so averaging energies would average
    logarithms and give the geometric rather than the arithmetic mean of the two
    probabilities. Recovering p uses the fact that it sums to one.

    The constant is then restored from the source table rather than recomputed
    from the symmetrized distribution. The shipped tables are not on a common
    convention -- the partition function is 680 for glycine and 134 for alanine
    -- so rebuilding the offset from the new table's own entropy silently
    rebases glycine against every other residue, and the ref weights were fit
    against the originals. Preserving the source partition function keeps
    sum(exp(-E)) unchanged, which is what puts the replacement on the scale of
    the table it replaces.
    """
    source = numpy.exp(-energies.astype(numpy.float64))
    partition = source.sum()
    p = source / partition
    p = 0.5 * (p + mirror(p))
    p /= p.sum()
    return (-numpy.log(p) - numpy.log(partition)).astype(numpy.float32)
