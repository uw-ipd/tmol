...
enable_cuda=${TMOL_ENABLE_CUDA_BUILD:-ON}
...
apptainer exec --bind "${source_dir}:/work" --pwd /work "${image}" \
    ...
    -Ccmake.define.TMOL_BUILD_TESTS=OFF \
    -Ccmake.define.TMOL_ENABLE_CUDA="${enable_cuda}"
...
grep -Fx "TMOL_ENABLE_CUDA:BOOL=${enable_cuda}" "${source_dir}/CMakeCache.txt"
apptainer exec --bind "${source_dir}:/work" --pwd /work "${image}" \
