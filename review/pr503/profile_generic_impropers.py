"""Paired warm generic-scoring cost of connection-improper ownership checks."""
...
def main():
    ...
    source = root / "tmol/score/genbonded/potentials/genbonded_pose_score.impl.hh"
    ...
    # Compile only the old genbonded kernel under a distinct C++/torch
    ...
    baseline_sources = args.baseline / "tmol/score/genbonded/potentials"
    build = args.output.parent / "genbonded-baseline-native"
    ...
    for source_file in baseline_sources.iterdir():
        ...
        text = re.sub(r"<tmol/score/genbonded/potentials/([^>]+)>", r'"\1"', text)
        text = text.replace(
            "namespace genbonded {", "namespace genbonded_review_baseline {"
        )
        text = text.replace("genbonded::", "genbonded_review_baseline::")
        text = text.replace(
            "TORCH_LIBRARY(tmol_genbonded,",
            "TORCH_LIBRARY(tmol_genbonded_review_baseline,",
        )
        target = build / source_file.name
        if not target.exists() or target.read_text() != text:
            target.write_text(text)
    baseline_ops = load_ops(
        "review_genbonded_baseline",
        str(build / "loader.py"),
        [
            "compiled.ops.cpp",
        ...
        "tmol_genbonded_review_baseline",
    )
    rows = []
    ...
    for batch in args.batches:
        ...
        term = GenBondedEnergyTerm(db, device)
        ...
        scorers = {
            ...
            "baseline": TermWholePoseScoringModule(
                ...
                pose,
                attributes,
                baseline_ops.genbonded_pose_scores,
            ),
        }
    ...
    result = dict(
        ...
        baseline_native_sha256=hashlib.sha256(
            (baseline_sources / "genbonded_pose_score.impl.hh").read_bytes()
        ).hexdigest(),
