...
| Prepare and inject ligand parameters | {doc}`Ligand workflow </user_guide/ligands>`; {doc}`Tutorial 07 <07_ligand_and_params>` | Start from authoritative CIF/MOL2 chemistry. `.tmol` is the only parameter format; Rosetta `.params` is not read or written. |
